...
**Acceptance:**
- [x] A live-shaped action runs the full chain and persists `prompt_version='v12'` + per-stage stamps.
- [x] The legacy machine still compiles and its tests still pass (it is dead-but-present, not deleted).
- [x] Non-terminal combat beats apply their mutations and persist relations across beats (the multi-beat half of the scene-state spine, deferred from Task 0), with round N persisted before round N+1 begins.
- [x] `step()` is serialised per action — a test proves two concurrent steps on one action cannot interleave its combat state (the [[T3-followup-combat-bail-spec]] caveat).
**Verification:** an engine test runs one action through the pipeline against a seed; suite + typecheck green.
**Files:** `src/engine/WorldEngineImpl.ts`, tests. **Scope:** M. **Deps:** T2, T3, T4, T5. **Landed** — `WorldEngineImpl` constructs `PipelineActionStateMachine` with `ProdPipelineLlmGateway` + `config.critic` (5th ctor param) when `config.pipelineLlm` is present; the sim's legacy path omits it and gets the v11 `ActionStateMachine` (unchanged). `src/index.ts` passes `pipelineLlm` when `DEEPSEEK_API_KEY` is set. Per-stage stamps route through the gateway's `LlmCallRecorder`; `actions.prompt_version` writes `'v12'` (dynamically — `'v11'` on the legacy path). Multi-beat persist in `stepActionPipeline` mirrors `PipelineSimEngine.stepAction` (:195-221): non-terminal `result.mutations` apply via `applyMutations` + `persistAuthoredRelations` before the next beat's context is built. `step()` serialised per action via `steppingActions` Set — a test races two concurrent `stepAction` calls against a mock-fetch-hung `ProdPipelineLlmGateway` and asserts the second throws. **Refund caveat confirmed**: bail/systemRefund logic in `stepActionPipeline` + timeout refund in `resolveStaleTimeout` (widened parameter type) fire on the pipeline path, not just legacy. Legacy `ActionStateMachine` + `PROMPT_VERSION` stay in-tree uninvoked; all 1203 tests pass (1202 baseline + 1 serialisation test).
