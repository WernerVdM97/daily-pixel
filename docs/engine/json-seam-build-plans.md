...
[ ] **M3.2 — Action choice + option-resolution death.** Absorb `handleActionChoice` (`action.ts`) into the controller; the engine absorbs option-resolution into `lastActionState.pendingDecision` (parent decision 1); the `pendingDecisions` map + `getChoiceLabel` + `setPendingDecision` are deleted. Reuses the M2 `Decision`/`OutcomeViewState`. Core slice; touches `action.ts` heavily.
...
[I] **Controller emits ViewState + semantic outcome facts (DC-C).** Extend the constructor to `new SessionController(engine, getCurrentScene)` (`getCurrentScene` is `discord.js`-free). Add `beginChoice(userId, selector): { kind: 'no-character' } | { kind: 'expired' } | { kind: 'resolved'; label: string; character: CharacterData }` (does the `getCharacter` guard then `engine.resolvePendingChoice`; `expired` only when the option path returns null) and `async stepChoice(userId, label, prevChar): Promise<StepChoiceResult>` where `StepChoiceResult = { kind: 'decision'; view: DecisionViewState } | { kind: 'outcome'; view: OutcomeViewState; distilledType: string; actionId?: number; characterName: string; characterClass?: string | null; char: CharacterData | null; prevChar: CharacterData } | { kind: 'error'; message: string }`. `stepChoice` calls `engine.stepAction(prevChar.id, label)`; on resolved it re-reads `getCharacter(userId)`, builds the `OutcomeViewState` via `buildOutcomeView(outcome, char, getCurrentScene(userId), state, undefined, engine)` (NOT compact, matching the current `applyActionResult`) and returns the outcome facts; on non-resolved it builds `DecisionViewState` via `buildDecisionView(nextDecision, state.decisions.length, state, char ?? undefined)` (no `actionType`/enemy args on a CONTINUE beat); on throw it returns `{ kind: 'error', message }`. Imports `buildDecisionView`/`buildOutcomeView` from `src/view/actionViewState.ts`; never imports `discord.js`.

[I] **Adapter rewrite keeps all transport + payload + broadcast (DC-D).** Rewrite the `action:` leaf (`dispatchInteraction.ts` around line 761) to drive the controller, byte-identical to today's `handleActionChoice` + `applyActionResult` ordering: build the selector (`CID_BAIL` -> bail, else `parseActionCid` -> option, null-parse returns early); `beginChoice`; `no-character` replies (ephemeral, before any defer); `deferUpdate`; `expired` paints the "session expired" `webhook.editMessage`; resolved paints the inline "**You:** {label}\n\n⏳ **Thinking…**\n_{idle}_" embed (built inline, unchanged, since `LoadingViewState` is M3.4); then `stepChoice` -> `error` paints "⚔️ Action Failed", `decision` paints `webhook.editMessage(decisionViewToDiscord(view))`, `outcome` paints `webhook.editMessage` with `outcomeViewToDiscord(view)` + `getNavButtons`/`getOutcomeServiceButtons`, assembles the public `payload` (content line via `classEmoji`, `getPublicOutcomeButtons`), then `broadcastOutcome` + `announceCollapse`. The outer error funnel (`notifyAdmin` + generic reply, lines 772-783) stays wrapping the whole leaf (Bucket A / decision 5). All `discord.js` verbs, payload assembly, and the broadcast/announce mechanics stay adapter-side.
...
M3.2a — Engine option-resolution (own commit):
[ ] Add `PendingChoiceSelector` + `resolvePendingChoice` to `WorldEngine` (interface), `WorldEngineImpl` (per DC-A), and `MockWorldEngine` (canned per DC-F). No caller yet: behaviour unchanged.
[ ] New engine unit test `tests/engine/resolve-pending-choice.test.ts`: happy option-index, bail-find, empty-options Continue fallback, no-state (bail -> 'Bail', option -> null), out-of-range index -> null.
[ ] Verify: typecheck clean; `npm test` green at 81+ files / 1512+ tests (only additions, zero snapshot churn).

M3.2b — Relocate view-builders (own commit, verbatim):
[ ] Create `src/view/actionViewState.ts` per DC-B; move the builders + exclusive helpers + choice/bail cid vocabulary verbatim; `action.ts` re-imports; medium wrappers + `withNarration` stay.
[ ] Verify: typecheck clean; `npm test` green with ZERO snapshot churn (`git diff --stat` shows nothing under `tests/discord/__snapshots__/`); confirm `src/view/actionViewState.ts` has no `discord.js` import (direct or transitive).

M3.2c — Controller + adapter + deletions + oracle (own commit):
[ ] Controller `beginChoice`/`stepChoice` + constructor extension (DC-C); wire `getCurrentScene` at the `index.ts` construction site + the harness.
[ ] Adapter rewrite of the `action:` leaf (DC-D); delete dead state + dead `setPendingDecision` calls + unused imports (DC-E); rewire the oracle + harness (DC-F).
[ ] Verify: typecheck clean; `npm test` green with ZERO snapshot churn; `grep -rn pendingDecisions\|getChoiceLabel\|setPendingDecision src/` returns nothing; the oracle's two `action:choice` transcripts still assert the resolved label.
