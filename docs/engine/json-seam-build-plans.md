...
[ ] **M3.0 — Dissolve the M2 import cycle (mechanical, first).** Relocate `clip`, `MAX_EMBED_DESC`, `outcomeColor` out of `src/discord/commands/action.ts` into a neutral module (proposed `src/render/embedText.ts` — all three are pure: `string→string`, a `const`, `string→hex-number`, no `discord.js`). Both `action.ts` and `src/discord/viewToDiscord.ts` import **down** from it; the `viewToDiscord` ↔ `action` cycle dissolves. Pure relocation, no behaviour change — the oracle covers it for free. Own commit; lands before any flow moves.
