...
[ ] **M3.1 — Controller skeleton + feedback/bug submissions.** Stand up `src/controller/` and port the four modal-submit leaves (`sleep:feedback:modal`, `release:feedback:modal`, `outcome:feedback:modal`, `outcome:bug:modal`) plus their modal-open buttons. These are the thinnest flow (adapter collects text → controller calls `submitFeedback`/`submitBug`), so they establish the controller pattern, the `NoticeViewState` reply, and the modal round-trip protocol at lowest risk.
