...
[x] **M0 — Commute rule into the engine.** *(done 2026-07-18, commit `98a4de1` — plan + execution state in [[json-seam-build-plans]])* The engine's work-action path owns "at the Oak → move to workplace, −1 stamina"; the handler only renders the outcome. Deletes the sole direct `charRepo.update` in the Discord layer (`src/index.ts`, the commute block). Covered by engine tests; `sim/` exercises it for free.
[x] **M1 — Behavioural oracle.** *(done 2026-07-19, oracle `2a3a069` + resolved-path fix `0bd0401`, on the minimal DI hoist `b9b4c4a` — plan + execution state in [[json-seam-build-plans]])* Golden-transcript characterisation of all 17 dispatcher leaves (incl. the resolved/outcome render path), cascade order pinned; diffable after every extraction step. Hard prerequisite for M3, now met.
[x] **M2 — Semantic view-state DTO + shared renderers.** *(done 2026-07-19, build `2275a42` + review fix `a04ede2` — plan + execution state in [[json-seam-build-plans]])* The transport-neutral view-state DTO (`src/view/viewState.ts`) and the single medium step (`src/discord/viewToDiscord.ts`) are defined; `buildDecisionMessage` / `buildOutcomeEmbed` assemble the DTO then map it, so Discord output is byte-identical (M1 oracle + action snapshots unchanged). The shared seam M3/M4 emit into.
[x] **M3 — Controller extraction, screen-by-screen.** *(done 2026-07-20, slices M3.0–M3.7, commits `825ae41`…`d9e96a0` — plan + execution state in [[json-seam-build-plans]])* A transport-neutral `SessionController` (`src/controller/`, never imports `discord.js`) absorbed the dispatcher's game-flow (action-choice resolution, the day-job menu + work flow, custom actions, feedback/bug submits, the character-gate reroute, last-played stamping), emitting `ViewState` DTOs the adapter paints; the engine absorbed option-resolution (`resolvePendingChoice`, decision 1) so the `pendingDecisions` map is gone. The dispatcher is now transport + paint only — its residual `engine` reads are nav-button payload assembly and broadcast routing. Byte-identical every slice (M1 oracle + M2 snapshots unchanged, zero churn); `join:` stays adapter-delegation.
[x] **M4 — Agent-player adapter.** *(done 2026-07-21, slices M4.0–M4.6, commits `6d03890`…`384309e` — plan + execution state in [[json-seam-build-plans]])* An in-process, Discord-free peer adapter (`src/agent/`, imported by neither `discord/` nor `sim/`) that stands up a real `WorldEngine` + `SessionController` and plays the whole game across days with a real DeepSeek brain, all features on — the "whole game, shorter horizon" harness (decision 6), for both bug-hunting/QA (exceptions, dead-ends, illegal-move loops, and invariant breaches captured as structured transcript findings + a run scoreboard; a throwing run still writes a repro) and playtest feedback (a critic LLM turns a completed transcript into a qualitative pacing/clarity/fun/difficulty report). It calls `SessionController`'s existing typed methods in-process (decision 3 — no JSON event-router built, no Discord rebuild); the three Discord-only bookends (character creation, nightly rest+tick, `/hi`) go engine-direct (DA-4). Opt-in via `npm run agent:play` (needs `DEEPSEEK_API_KEY`); `npm test` uses deterministic stubs, never the network. The scope fence held — no engine/controller/discord/sim source changed (the M3 seam was reused as-is).
