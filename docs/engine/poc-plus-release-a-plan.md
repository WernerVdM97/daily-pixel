...
- [ ] **(SL-7)** A spared foe's fight edge is closed, so the next combat action *establishes* a fresh fight rather than continuing at 1 HP; a test drives spare → re-engage and asserts the new fight starts at round 1 against a foe of `ENEMY_HP_MIN` HP, not 1 HP. The old RA-5c `enemyHp: 1, round: 1` edge assertions are replaced, not merely relaxed.
- [ ] **(SL-7)** The terminal beat after a spare still reports the foe at 1 HP, so the outcome frame bands a wounded tier and never `'Slain'` — i.e. the beat and the edge deliberately disagree, and both are pinned.
- [ ] A spared *resolved* NPC has its row left untouched (no health write), and the fight still closes.
- [ ] Also in this commit: correct the stale hand-built fixture in `tests/discord/action-decision.test.ts` that gives the fatal-blow interstitial `woundWord: 'Slain'` / `filled: 0`. The real path cannot produce that (it bands a nominal 1 HP), so the fixture misleads the next reader about what the screen shows.
