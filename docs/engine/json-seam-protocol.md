...
[ ] **M7.2 — `/hi` through the seam.** *(slice checklist below, lead-settled 2026-08-05)* `hi.open` event; controller `openHi` composes the three screen variants as `NoticeViewState`; router branch + `addCharacterFacts`; `hi.ts` becomes translate + paint. Consuming adapter in-slice: the rewired `hi.ts`. Contract tests in the same commit.
