...
[ ] **M8 — Read-only screens through the seam.** *(plan below, lead-settled 2026-08-06 — M8.0 screens oracle first, then one crossing slice M8.1 for all screens, per the recorded M7.3→M8 coordinator steer)* `look`, `map`, `stats`, `backpack`, `journal`, `help` become `screen.*` events returning view-states; the agent gains full player parity on the reachable surface.
...
[ ] **M8.0 — Screens oracle.** *(slice below — the only slice fully specced today)* Test-only, additive: the golden-transcript net over the six screens' slash + nav arms, the charless edges, look's null-location branch, and the gate reroute. Gate: new transcripts + snapshots green, existing oracles + suite untouched, zero `src/` edits.
[ ] **M8.1 — One crossing slice for all screens.** *(checklist written when M8.0 lands, per the write-checklists-as-you-go pattern)* Six events + validator cases (DC-M8.1), controller `open*` methods + composition moved into `src/controller/*Screen.ts` modules (DC-M8.3/5/6), router branches + copy constants (DC-M8.4), the six command files rewire to translate + paint, `index.ts` + `dispatch-harness.ts` wiring (the controller gains `resolveScene`), contract suite extended for all six events in the same commit, unit suites rewire (formatter imports move; the no-stamp spies per DC-M8.7), changelog. The map slash-arm focus extraction stays adapter-side in `index.ts` until M9 (the registry wrapper already does it).
