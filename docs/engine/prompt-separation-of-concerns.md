...
- [ ] An action runs the chain: a typed classification routes to the right template; decide emits options (and scene-framing `narration` on CONTINUE beats per [[decide-scene-narration]]); a fresh resolve stage produces mutations from a structured handoff — every row stamped with the exact `v12` prompt-set version.
