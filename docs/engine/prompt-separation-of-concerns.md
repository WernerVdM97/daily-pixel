...
- [x] **Settle prompt-set versioning** — asset layout + `PROMPT_VERSION` shape for a multi-template set (`decision-prompts/v12/{classify,…,resolve}.md`). Gates all of Thread D; decide before building (see [[prompt-v12-pipeline]]). Settled in [[v12-prompt-set-versioning]]; scaffolded in `src/llm/prompt-builder.ts` (loader + stubs).
