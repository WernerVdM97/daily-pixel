...
3. **`combatCapScenario`** — high-stat char (physical 15 → bonus +2, health 20, maxHealth 20), `baseDc 12`, roll sequence `[8,8,8,8,8,8,8,8,8,8]` (equal totals → trade every round; 4 fought + 1 cap-derive step). choicePolicy `first-real`. After 4 trades enemyHp 12→4, playerHp 20→12; `playerFrac 0.6 ≥ enemyFrac 0.333` → `success`. Expected `combatMetrics`: `{ roundsFought: 5, floorSaves: 0, wins: 1, losses: 0 }`, turn outcome `success`.
