...
**Superseded 2026-09-14:** the DeepSeek rows above are the 09-10 state. Every DeepSeek pin now reads `openrouter/deepseek/deepseek-v4.1-flash` at `xhigh`, and the direct provider is pinned by nothing. The direct route is being retired rather than kept as a spare, so no `fallbackModels` is wired; see the reversal note under Model facts.
...
- `deepseek/deepseek-v4.1-flash` was the intended triage model but is not in the active Pi registry (owner rule: direct DeepSeek only, never via OpenRouter), so triage runs on `deepseek/deepseek-v4-flash-vision-exp` — same direct provider, same image input, same `xhigh` ceiling.
  - **Reversed 2026-09-14.** The direct-only rule is gone: every DeepSeek agent pin now takes the OpenRouter route `openrouter/deepseek/deepseek-v4.1-flash`, held to the DeepSeek first-party host by the `only: ["deepseek"]` / `allow_fallbacks: false` override in `~/.pi/agent/models.json`. A routed model served by whichever host is cheapest that minute gets a cold prefix on every switch, so fixing the host is what protects the cache-read rate. Two consequences worth keeping: the OpenRouter entry takes text **and** image, so triage's image ingestion survives the move, and its thinking map is `off`/`high`/`xhigh` with no `max`, so `max` pins became `xhigh`. GLM pins are untouched. The direct route is deprecated, not spare capacity: a drained OpenRouter balance or an unavailable DeepSeek host stops the factory, which is the accepted trade for one balance and one prefix. The table above and the paragraphs below are left as written; the agents themselves are the living artifact.
