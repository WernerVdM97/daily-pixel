...
---
_Build plan for the `0.3.2` release: a second interstitial cut, still before Stage 2 (nat 1/20 broadcast). It exists because the 2026-07-11 prod+dev review (two testers, one build after 0.3.1 shipped) showed the combat-readability release backfired: the continue card reports wins as losses, the WIN verdict contradicts the band and the HP outcome, and the foe shows as a generic "Minion" with a DC-derived HP bar that grows between rounds. This release makes the combat readout tell one true story, then folds in the fresh non-combat bug batch. Executor-grade contract for an orchestrated-delegation lead; anchors were verified 2026-07-11 and will drift, so the lead re-verifies each while scouting before delegating._
...
`0.3.1` shipped the combat maths reveal and the combat-frame redesign to make fights legible. The live review immediately after (prod snapshot `warden-20260711-185823`, tester BendiusOver; local dev DB, tester trombosis) shows it made combat *less* trustworthy, not more, because the numbers on the card do not reconcile. Players rolled a 22 against a shown "DC 15" and were told they lost; rolled a 12 against "DC 15" and were told they won; saw `+ WIN`, `TRADE`, and an HP loss stacked in one frame; and fought a 24-HP stag that rendered as "Minion 6/14" whose bar grew each round. Broadcasting crits publicly (Stage 2) on top of a card that mislabels wins would amplify the damage, so the combat readout has to become correct and coherent before that work starts. This release is combat-correctness first, then the fresh non-combat bug batch from the same review.
