...
| Meta-oil | `meta-oil` | Fri + Sat 18:00Z (20:00 local) | No — friction analysis and numbered proposals in one linked digest: the content is the index, the embed carries a card per proposal; edits a factory file only once that exact proposal is approved |
...
**Its digest is one message in two parts**, because a plain body has a single 2000-character budget for everything and a proposal card competes with every other card for it: the content carries the index (signals, window, spend, what is still open, and the reaction legend) under 900 characters, and one embed carries a card per proposal, six labelled lines each. Every issue, PR and file it names is an absolute link; `.pi/factory/memory/` is never linked, because it is gitignored and the link would 404. The six fields are `signal`, `why`, `files`, `diff`, `verify`, `blast` (`.pi/agents/meta-oil.md` is the shape, and `scripts/send-dm.ts` refuses an over-long field by name and count the way it refuses an over-long body). Scrumo stays on plain content on purpose: three decisions need no cards.
...
To answer a meta-oil digest, react on the DM it sent. If the run could not record its own message id (see `scripts/factory-inbox.ts --record <id>`), the watcher says so instead of quietly reporting no decisions. The digest is content plus an embed, sent as one message: `npx tsx scripts/send-dm.ts -f content.txt --embed embed.json` (`--embed` takes one embed object or an array).
