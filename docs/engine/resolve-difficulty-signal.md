...
So combat gets the danger tier, not a DC. That choice has a second payoff beyond honesty: the card and the narration agree on the same word for every round of a fight, instead of the prose and the UI describing difficulty on two unrelated scales.
...
**Open — the tier ladder still predates v13.** `dangerTier`'s thresholds (easy ≤9, medium ≤13, hard ≤17, risky ≤21, fatal >21) were anchored against the **pre-v13** ladder, with the comment at `combat-dc.ts:225-227` citing "the sim's baseline goblin (baseDc 12) reads medium". A legal `baseDc` is 10-24 (`decide/BASE.md`), so `easy` (≤9) is unreachable for a contract-honouring fight: the tiers in practice are medium (10-13), hard (14-17), risky (18-21) and fatal (22-24). Note "in practice" is doing real work there. That range is a **soft** contract: `validateDcModifier` bounds only the ±5 modifier and nothing clamps an authored `baseDc` at all (`critic-gate.ts:62-64` says so outright, which is why its anomaly band exists), so a model that authors 8 does produce `easy`. That is why the recipe still lists all five tier words and scales only on the four reachable ones, rather than pretending `easy` cannot arrive. `decide/BASE.md:31` anchors an ordinary action's `baseDc` at 16-17, which reads `hard`, not `medium`, so an ordinary fight lands on the third of four reachable tiers rather than the first. The words remain individually sensible and this change only makes an existing display ladder audible in prose, so re-tuning it is a balance decision with its own measurement, not part of wiring the signal. Logged for whoever tunes combat next.

The card/narration match above was per-round, not per-fight. `PipelineActionStateMachine.ts` replaced `state.lastDecideResult` with each CONTINUE round's fresh decide result, including a newly authored `baseDc`, and nothing pinned the fight's `baseDc` at establish time, so in a multi-round fight whose later round authored a different `baseDc` the tier RESOLVE received reflected the final round rather than the tier the card showed on the round the player last saw before that. **Fixed in #97** (the owner decision recorded on that card, 2026-09-22, which also flipped this doc's own residual): the opening round still authors `baseDc`, and the engine now persists it as a `baseDc` prop on the `in_combat` edge, which every CONTINUE round reads back — so `enemyBonus`, the card's `dangerTier` and the `foeDanger` RESOLVE is handed are one number for the whole fight. The `decide` message says so too: a combat CONTINUE carries `- Fight DC: <n> (the fight's own DC, fixed when it opened; any baseDc you author this round is ignored)` beside the round summary, so the model is told the number rather than invited to move it, and the wording does not contradict `decide/BASE.md`'s requirement that every beat still author a legal `baseDc`. The continuing `dangerTier` threshold residual above is #96.
