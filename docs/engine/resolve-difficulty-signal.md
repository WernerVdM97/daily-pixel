...
The card/narration match above is per-round, not per-fight. `PipelineActionStateMachine.ts:890` replaces `state.lastDecideResult` with each CONTINUE round's fresh decide result, including a newly authored `baseDc`, and nothing pins the fight's `baseDc` at establish time. In a multi-round fight whose later round authors a different `baseDc`, the tier RESOLVE receives reflects the final round, not the tier the card showed on the round the player last saw before that. This is a pre-existing property of how combat threads `baseDc` per round; this change only makes it audible in prose for the first time. Pinning `baseDc` for a fight's duration would change `enemyBonus` and `enemyMaxHp` behaviour round to round, so that is a balance decision needing its own measurement, not part of wiring the signal.
