...
**Acceptance:**
- [/] A forced classify-stage throw produces: a refunded roll, no mutations, and an outcome that names itself a system failure with distinct visual treatment. _Regression test added (`tests/engine/world-engine-impl.test.ts` — "classify-stage throw → divine intervention (F#21)") and covers the DB-level refund (roll count unchanged) + `outcome.mutations` empty. BUT a fresh-context review found the **presentation half is broken in prod**: the `⚠️ System` embed (`action.ts:262-273`) is dead code — the `if (result.outcome)` auto-finish branch at `:220` intercepts divine first (divine returns `outcome`+`firstDecision` together, `WorldEngineImpl:958-962`), so divine renders as a plain grey `✅ DONE` outcome via `buildOutcomeEmbed`, showing `🎲 (-1)` (looks like a spent roll: `rollsDelta=-1` at `WorldEngineImpl:957` fires for divine too, `rollRefunded` never set). Criterion (c) "distinct visual treatment" + the refund display are unmet. **Fix pending before this box is ticked.**_
