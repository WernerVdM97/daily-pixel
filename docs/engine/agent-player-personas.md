...
- [x] **T8 · the interrupted panel.** `skipDays(n)` on the harness via `advanceDays` + `pinClock` (one calendar day per skipped tick), and the absence-then-return path. _(Machinery and unit coverage shipped; no live interrupted run yet.)_
