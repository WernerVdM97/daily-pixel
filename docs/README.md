...
| ✅ | [RESOLVE difficulty signal](./engine/resolve-difficulty-signal.md) | Why RESOLVE receives the difficulty it was already instructed to reward, and why one field would have been wrong: a DC-checked attempt carries its final DC, a fight carries the worded danger tier the combat card already renders, and an auto-resolve carries neither. Two balance residuals logged, one since closed: `dangerTier`'s thresholds predate v13's ladder (open, #96), and the card/narration tier matched per round rather than per fight (fixed, #97). |
...
| 🔭 | [OpenRouter keys and cache observability](./engine/openrouter-keys-and-cache-observability.md) | Who spends on which OpenRouter key (four actors, two of them sharing one today) and why the host pin added with the route change cannot be verified yet. Records the credential-precedence trap that makes an exported env var useless (`--api-key` beats `auth.json`, which beats the environment), the `$5`-a-day cap the factory key carries and the `limit_remaining` read that makes a spent cap a skipped tick rather than a 401 storm, and the five `llm_calls` columns that turn "is the pin holding and is the cache warm?" into a query: `provider` is already returned on every response, so unlike the pi-side `cachestat` pattern this needs no id-to-host backfill. |
| 🔭 | [Dark Factory](./engine/dark-factory.md) | The continuously-triaging, human-gated agent factory that runs this repo: the GitHub Projects "Dark Factory" board as control plane, the scheduled loops (triage / executor / sweeper / scrumo / meta-oil, all enabled) plus the executor-spawned escalator, with their topic-scoped memory under `.pi/factory/memory/`, and the approval gate (agents only execute Status=`Approved` or an `auto:*` class; merging, releases, and `dev`/`main` stay human). Board seeded from TODO.md 2026-08-03. |
| ✅ | [Dark Factory job ledger](./engine/dark-factory-job-ledger.md) | One board item becomes a tracked job (`.pi/factory/jobs/<item>.json`) whose stages run one per process with their own budgets: build → review → fix → deliver → reconcile → done, 100 minutes total. Moves the gate, the delivery step and the merge reconciliation into code, so a killed run can no longer leave an item `In Progress` with no PR, a second failure blocks and pages instead of retrying silently, and a merged PR closes its own card and issue. |
| ✅ | [Dark Factory requirements (scrumo + model tiering)](./engine/dark-factory-requirements.md) | The decisions behind the fourth loop, the escalator child and the per-loop model pins, built 2026-09-10; kept for the model facts (costs, thinking tiers, and the direct-provider rule it set, since reversed in favour of an OpenRouter route pinned to the DeepSeek host) and the resolved question on how far scrumo may write. |
| ✅ | [Agent-Player Personas](./engine/agent-player-personas.md) | The playtest panel: ten player-type presets (explorer, socialite, soldier, homesteader, grinder, collector, storyteller, tourist, casual, lapsed returner) that roleplay a gamer, see the screens a real player sees (`/map`, `/journal`, `/backpack`, recap, intent and arc notes), get recon moves that cost no roll, and judge the run against the game's own long-horizon promise rather than one day's fun. Reviews split **engagement** from **fulfilment**, tag friction by recurrence (a daily-ritual annoyance is a churn driver, a once-a-run one is trivia), and track a per-day engagement/fulfilment series so decay is visible. Panel runs in three shapes: breadth (all ten, one day), arc (a subset over a week), interrupted (absence, then return). Also the first realistic route to observing the RA-2 inspiration dial, since free-text-preferred personas are what a work-dominated run can never be. |
