...
| ✅      | [Stage 3 — Combat spine (plan)](./engine/stage-3-combat-spine-plan.md) | Handoff-ready build plan for first-class combat on the pipeline machine: an engine-owned spine (contested roll + severity bands, `combatState` as an `in_combat` scene-state edge, a once-per-day no-one-shot floor, a lifted round cap) plus combat prompt rules and an offline sim proof. Pipeline-only; engine owns every combat number, the LLM only narrates. Two-pass: Pass 1 = pure band math + combatState model then a human checkpoint; Pass 2 = round loop + floor + templates + sim. Hybrid enemy anchoring (npc for named/boss, location for minions) and no Thread-B dependency (a `scale` seam left at 1). |
| ✅      | [T3 follow-up — combat bail (spec)](./engine/T3-followup-combat-bail-spec.md) | Child task spec of Stage 3 T3: gives an engaged fight a voluntary "flee at a cost" option on every continuation round (round 1 stays flee-less), reusing the existing bail path so a bailed enemy is remembered at its anchor. |
| ✅      | [T3 iteration 2 — floor + loss ladder (spec)](./engine/T3-iteration-2-spec.md) | Child task spec of Stage 3 T3: the once-per-day survive-at-1 floor, the engine-authored desperate-choice beat (bail bloodied / last stand), and the loss ladder + `hpZero` marker for a second lethal blow the same day. |
| ✅      | [T4 — v12 combat prompt templates (spec)](./engine/T4-combat-templates-spec.md) | Child task spec of Stage 3 T4: encodes the §C-a combat rules (danger keyed to location safety, item-anchored options, `required: true` throughout, `combatEnemy` identity signalling) into the v12 decide/resolve combat templates, authoring only ancillary mutations around the engine-owned combat numbers. |
| ✅      | [T5 — combat telemetry (spec)](./engine/T5-combat-telemetry-spec.md) | Child task spec of Stage 3 T5: per-round `CombatBeatLog` telemetry (band, enemyHp before/after, material-mutation signal), sim `combatMetrics` aggregation, and the win/floor/cap-derive sim combat scenarios — the data source for the parked prose-critic trigger. |
