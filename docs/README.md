...
| 🚀     | [Polish Pass — v0.2.7](./sparks/polish-v0.2.7.md)                                                              | **Shipped** (`[Unreleased]`, branch `feat/polish-v0.2.7`). POC-beta polish from the dev-DB tester feedback/bugs: UI/emoji wins (`/hi` glyph, `/look`+`/map` path emojis, `/join` stat icons, `/backpack` rails), one-free-bail-per-day, `/hi`→join reroute, the Saturday-threat dawn heads-up, and the universal **≤1-option degenerate-shape guard** (retry-then-bail, Bug #2). Stamina clamp deferred to v11 (per-action-type caps need the `category` enum); travel cost, the weekly poll, and the Saturday-roll question parked. |
| ✅     | [Polish Pass — Follow-up (post-0.2.7)](./sparks/polish-pass-follow.md)                                         | **Decided** — bug-only plan from the dev-DB feedback/bugs still open after `0.2.7`. Sequenced: verify-first (carried-buff application, `98 copper`), then the high-confidence outcome-footer bump (frontier-crossing move line + net-zero buff surfacing, both in `OutcomeRenderer.ts`), then map edge-bearing inversion (`/look` vs `/map`) behind a decision record. Backpack `12/10` already handled (cap→40, 10×4 grid). Roadmap/own-spark items (items depth → [[improved-item-features]], party buffs, fast-travel, Discord `@mention`, auto-resolve *shaping*) pulled out of scope. |
