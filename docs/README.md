...
| ✅      | [POC+ Stage 1 — v12 tail, welcome tag & combat maths reveal (build plan)](./engine/poc-plus-stage-1-plan.md) | Executor-grade build plan for [[poc-plus-roadmap]] stage 1: T0 closes the v12 tail by reference (F#21 divine-intervention refund/signalling per [[prompt-v13-roadmap]] §4; the T7 dead-code sweep per the archived cutover plan), T1 ships the welcome tag (owner mention + 🌅 Hi button on the new-hero broadcast, pings suppressed), T2 builds the `AnsiRenderer` (colour-by-role, slot frames, char-budget tests) and reworks combat outcomes to show the maths (dice, margin, clamped per-combatant HP bars, floaters; B#5/B#6 as acceptance). Scope fences, verification baselines, release cuts, and the resume one-liner included. |
| ✅      | [POC+ 0.3.1 — combat-readability polish & hardening (build plan)](./engine/poc-plus-0.3.1-polish-plan.md) | Executor-grade build plan for the `0.3.1` interstitial release between stage 1 and stage 2: finishes the combat-readability arc by paying down the T2 live-check debt (the `ANSI frame polish` block: settle SGR/palette/glyph facts, standardise the renderer off black `chrome` + palette module, decouple render from engine, per-round combat maths visibility + card mocks, opening frames for all seven types + art-post/reply delivery), plus the 2026-07-08 prod bug batch (B#1-B#4) and a run of small UX wins. Bundled large so stage 2 opens on a clean renderer. Ordering, dependencies, live-check batching, scope fences, and the release-cut checklist included. |
...
| 🔭     | [Action Features & Art Integration Tracker](./engine/action-features-tracker.md) | Compact cross-reference of every built action feature (7 types, 17 mutations, 4 pipeline stages, 8 scene-state relTypes, combat surface) against ANSI art register coverage and the [[poc-plus-roadmap]]. Art status: 0 of 7 action types have ANSI art; renderer not yet built. |
