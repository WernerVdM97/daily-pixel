...
| 🔭 | [Dark Factory](./engine/dark-factory.md) | The continuously-triaging, human-gated agent factory that runs this repo: the GitHub Projects "Dark Factory" board as control plane, four paused loops (triage / executor / sweeper / scrumo) plus the executor-spawned escalator, with their topic-scoped memory under `.pi/factory/memory/`, and the approval gate (agents only execute Status=`Approved` or an `auto:*` class; merging, releases, and `dev`/`main` stay human). Board seeded from TODO.md 2026-08-03. |
