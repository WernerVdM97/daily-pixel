...
| Status | Doc | Summary |
|---|---|---|
| 🌱 | [Decision Prompt v9 — Markdown Input & Coherence Critic](./sparks/prompt-v9-markdown-and-critic.md) | **Next `0.2.x` patch.** Two LLM-layer threads spec'd for handoff, no mechanics touched: (1) JSON→markdown LLM input (the character-data template is the heart of it); (2) a coherence critic — a second LLM pass that reviews/repairs each decision before it's sent. Combat, world-scaling, and the full pipeline are all deferred to v10. |
| 🌱 | [Decision Prompt v10 — Combat, World Scaling & Multi-Stage Pipeline](./sparks/prompt-v10-scaling-and-pipeline.md) | **POC round 2 (`0.3.0`) kickoff**, builds on v9. The engine-heavy half: (C) combat as a long, frequent, high-reward wilds mode backed by engine scene-state (`combatState`, contested rolls, severity bands, no-one-shot floor); (B) the world scales around the player via a week-indexed World Tier; (D) decompose the mega-call into a classify → decide → resolve pipeline of per-type templates over graph-shaped scene-state (the v9 critic is its first stage). Sim harness is a prerequisite. |
