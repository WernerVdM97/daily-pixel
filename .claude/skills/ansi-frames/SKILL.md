...
For **data cards** (roll results, skill checks, stat readouts) follow the roll-card reference in the spark doc §12: no sprite art, five-beat typographic hierarchy (dim caps label, big focal number with right-aligned dim context, calculation line, colour-coded outcome with shape-redundant `✓`/`✗`, dim flavour), two colour switches total.
