...
3. **Stale branches.** Run `npx tsx scripts/factory-jobs.ts housekeeping` — the pruner, in code: it fetches, fast-forwards local `dev` when that is provably safe, and deletes local branches whose **PR is merged** (or that are wholly contained in `origin/dev`), never `dev`/`main`, the current branch, a branch checked out in a worktree, or one a live job record names. Report what it deleted, what it kept and why, and whether `dev` moved. Then list, for the owner, any branch it left behind that has no open PR and no commits in 14 days: those need a human. Never delete a branch yourself, and never touch `origin`'s branches: the tick runs the same command every 5 minutes, so what reaches you is usually just the report.
