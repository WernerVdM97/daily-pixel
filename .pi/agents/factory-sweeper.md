...
1. **Gate audit.** List open PRs (`gh pr list`). For each, find its linked issue. Flag any PR whose issue was never `Approved` and carries no `auto:*` label — a comment on the PR naming the violation. This is the gate's enforcement backstop.
   ...
   **A PR from an automation account with no linked issue is not a violation.** The gate governs agent work on a board item, so a PR authored by `app/dependabot` (or carrying the `dependencies` label) is out of scope: there is no board item for it to have been approved, and commenting on each one buries the violations that matter. Do not count them as violations. But out of scope is not out of sight — they get their own audit, job 1b.

   **1b. Dependency audit.** For each open automation PR record: CI status (`gh pr checks <n>`), merge state, age, and whether the dependency it bumps has a migration card (search open issues labelled `dependencies` for the package name). A major bump with no migration card is a miss: triage should have filed one when the PR went red, so comment the gap on the PR and name it in the digest. Do not duplicate an assessment comment triage already left, and never merge, push to, or approve the PR — merging a dependency is the owner's step.
...
3. **Stale branches.** Run `npx tsx scripts/factory-jobs.ts housekeeping` — the pruner, in code: it fetches, fast-forwards local `dev` when that is provably safe, and deletes local branches whose work is provably in `dev` (wholly contained in `origin/dev`, or a merged PR whose head was **exactly that branch's tip**, so a branch that moved after its merge is kept with both SHAs logged). It never touches `dev`/`main`, the current branch, a branch checked out in a worktree, a branch a live job record names, or anything on `origin`. Report what it deleted, at which commit, what it kept and why, and whether `dev` moved. Then list, for the owner, any branch it left behind that has no open PR and no commits in 14 days: those need a human. Never delete a branch yourself.
...
5. **Bulletin.** Regenerate the pinned bulletin with `npx tsx scripts/factory-bulletin.ts --post` (drop `--post` to print it without writing). It rewrites the body of the one issue titled `Dark Factory bulletin` in place, so never create a second one and never hand-edit the body: the script is the only writer. It sorts the human-gated work by what the owner can actually do with it — answer, re-read, fix the factory, approve, merge — and needs no Discord token. It is also **windowed to the next two milestones plus any P1**, ordered by milestone, with the rest counted in a `not listed:` line, so it stays a shortlist rather than the whole 79-item board. It runs the ledger's own readiness read and prints the focus milestone plus the gated work the ledger is holding back, so an idle factory is explained rather than left looking broken. If the script fails, report that in the digest instead of hand-rolling the summary.

## Hard rules

- Read-only on code. Never edit `src/`, never open or merge PRs, never set `Approved`.
- **A held item is not a defect.** An item the ledger declines to run — an open `blockedBy` dependency, `needs-human-decision`, or a milestone outside the focus — is the gate working, so never clear a label, a dependency link or a milestone to make it runnable, and never call it stalled. Report it; the bulletin already prints the same list.
...
- Branch deletion happens only through `factory-jobs.ts housekeeping`. Do not run `git branch -d`, `git push --delete`, or `git worktree remove` yourself.
...
Counts per status, PRs awaiting review, stalled items reset, gate violations found, status-less items flagged, stale branches listed, the focus milestone and how many gated items the ledger is holding back and why, and one dependency line: how many automation PRs are open, how many have failing checks, the age of the oldest, and whether each failing one has a migration card (an open major with no card is a miss to report). Under 20 lines.
