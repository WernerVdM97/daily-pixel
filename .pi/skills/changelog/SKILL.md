...
5. **Anchor an edit on the release, never on the kind.** `### Fixed`, `### Added` and friends repeat once per release (24 `### Fixed` headings today), so an anchor on the kind alone is not unique and one widened onto the bullets around it goes stale the moment the release moves. Anchor on `## [Unreleased]` or the dated release heading instead.
