...
| Fixer | `delegate-fixer` | `openrouter/deepseek/deepseek-v4.1-flash` at `high` | accepted findings | implement only accepted changes |
...
**Model budget is the point.** The lower-reasoning tier proposes work and findings; the higher-reasoning tier accepts. Quality comes from process, not tier: fresh-context review, barrier verification, and lead triage gate what lands. Concrete models are pinned in each agent's `model:` and `thinking:` frontmatter under `.pi/agents/`: the mechanical roles ride `openrouter/deepseek/deepseek-v4.1-flash` at `high`, the loop starters and the sweeper ride `z-ai/glm-5.3-flash` at `low`, and only the judge and the escalator pay for `z-ai/glm-5.3` at `max`. **Every DeepSeek pin goes through OpenRouter, pinned to the DeepSeek first-party host** by the `only: ["deepseek"]` / `allow_fallbacks: false` override on `deepseek/deepseek-v4.1-flash` in `~/.pi/agent/models.json`. That override is the reason: a routed model served by whichever host is cheapest that minute gets a cold prefix on every switch, so the cache-read rate that makes a long session affordable only holds while the host is fixed. The direct `deepseek` provider is therefore unused by any agent pin. Mind the thinking-level maps when you change one: OpenRouter's `deepseek-v4.1-flash` offers `off`/`high`/`xhigh` and no `max`, the direct provider the reverse, and an unavailable level is clamped without complaint. The coordinator's value is position, not tier: a separate, read-only context that holds only the goal, the plan, and repo state. It is consulted at checkpoints, never per subtask.
