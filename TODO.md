...
## ⏭️ RESUME HERE — POC+ Release A (last touched 2026-08-01)
...
1. **The one open v13 prompt fix — the daunting band.** `baseDc` never reaches 20+ (0/21 measured) because `decide/BASE.md:29` and `:47` state the ladder on `base_dc` while the player rolls against `baseDc + dcModifier`, and the ±5 modifier cap makes a routine-to-daunting spread in one set need near-extreme modifiers the model doesn't produce. Restate the ladder on the final per-option DC. **v13 is not cut, so this lands as v13 — do not bump to v13.1.** Numbers and full reasoning: plan § Stage 4 measurement, finding 5.
...
- [/] **Stakes / difficulty pass** *(RA-1)* — item-bonus ceiling, the DC ladder and the reward/failure menus all landed (stage 4). The failure-cost rule measured 21/21 compliant, up from 0/12 on v12. **Residual: the daunting band is unreachable** — see § RESUME HERE item 1. No lethality (death track stays deferred). Overlaps the MVP "make wealth (and stamina, health) spendable/meaningful" item below.
- [/] **Inspiration dial** *(RA-2)* — all four prompt/engine halves landed (F#12 strip, named-reward line, the v13 frequency dial, the rest-channel fix). **Residual is measurement, not build:** the ~10% target and 3.2-3.7 band are unverified and the agent-player structurally cannot check them (§ RESUME HERE, standing cautions). Needs human play before anyone concludes it worked.
