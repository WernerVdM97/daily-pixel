...
**Landed: all of Release A's build work, stage 4 included.** RA-6, RA-4, RA-5, RA-3 bounded (+SL-6, SL-7), stages 1-3 of the RA-1/RA-2/v13 sequence (item-bonus ceiling, the `fatalBlow`/`decisionPrompt` handoff, the engine F#12 strip and the named-reward line), and stage 4 in full — steps 0-5 as `272d56d`, steps 6-7 as `d9d8268`, the review pass as `5162d16`. Per-task detail is in the plan's § Task log and `CHANGELOG.md` `[Unreleased]`, which is now current for stage 4.

**Next, in this order:**

1. **The one open v13 prompt fix — the daunting band.** The 2026-08-01 measurement found `baseDc` never reaches 20+ (0/21) because `decide/BASE.md:29` and `:47` state the ladder on `base_dc` while the player actually rolls against `baseDc + dcModifier`, and the ±5 modifier cap makes a routine-and-daunting spread in one set need near-extreme modifiers the model doesn't produce. Restate the ladder on the final per-option DC. **v13 has not been cut, so this lands as v13 — do not bump to v13.1.** Full reasoning and the numbers are in the plan's § Stage 4 measurement, finding 5.
2. **Release cut 0.3.3** then the doc loop, per the plan's § Release cut and § Doc loop. `poc-plus/release-a` is already merged into `dev` (local `dev` is at `8d0123f`, 9 commits ahead of `origin/dev`, **not pushed**). The lead never merges `dev` → `main`; prompt the user.
3. **Do not plan on the agent-player to validate the balance work** — § Stage 4 measurement, findings 1-2, explains why it structurally cannot. It remains a good QA/crash instrument.

**Overshoot risk to watch when the cadence is measured** (the plan's step 3 calls this "the stage 6 measurement", but there is no stage 5 or 6 — the measurement is the agent-player before/after run in § Doc loop; worth correcting in the plan)**:** RA-1 and RA-2 push the same direction at once and compound multiplicatively rather than additively. RA-1 raises DCs so more attempts fail, and RA-2 cuts the grant rate so there are fewer attempts per day: fewer turns, each likelier to cost than to pay. The RA-4 playtest critic already reported failure costs as harsh ("losing gold on a failed tail") and the bail mechanic as unrewarding, and that was on the *pre-change* tuning. If stage 6 lands at the bottom of the acceptance band, the honest reading is "stingy", not "tense". If one dial has to be relaxed, relax **RA-2's frequency dial rather than RA-1's ladder**: stakes are the release's stated purpose, cadence is a comfort setting. Note also that RA-1's anti-crush mechanism is a *rule* (every option set keeps one option in the routine band), not a softer ladder, so if playtests still read as crushing, check whether the model is honouring that rule before touching the numbers.

**The review sharpened that risk into an arithmetic one, and it points at undershoot, not overshoot.** Count the grant channels that actually survive in v13: `resolve/BASE.md`'s rare-and-earned rule, `skill/success.md`'s nat 20, and `rest/success.md`'s exceptional night. Two of those three now hang off a natural 20, whose probability is fixed at 5% and which is further diluted by each category's share of the action mix — so if the model reads the nat-20 case as the *only* licence (which the pre-fix BASE wording invited, and which `5162d16` deliberately widened against), the ceiling on the grant rate is 5%, giving `3 / (1 - 0.05) ≈ 3.16` rolls per active day — **below the 3.2 floor of the acceptance band before any dilution at all.** The `rest` narrative trigger is what has to carry the difference up into 3.2-3.7, and it is prose, so it is the model's judgement, not a guarantee.

**The 2026-08-01 measurement settled how this can and cannot be checked — read the plan's § Stage 4 measurement before running anything.** The short version: **the agent-player harness cannot measure RA-2 at all.** The brain overwhelmingly picks the day-job menu rather than free actions, and `stripWorkInspiration` (stage 3, F#12) removes inspiration from work actions by design, so the harness returned a 0% grant rate on **both** v12 and v13 — a structural zero, not a tuned one. Do not read that as the dial working, and do not read a future 0% as evidence either. RA-2 stays unverified until there is human play or a harness switch that forces free actions. What the same measurement *did* establish, from 59 isolated pipeline calls rather than the contaminated live runs: RA-1's failure-cost rule landed (21/21 forced failures carried a non-stamina cost, against 0/12 on v12), the anti-crush rule is honoured 21/21, and **RA-1's daunting band is unreachable in practice** (0/21 `baseDc` at ≥20) because the ladder is stated on `baseDc` while the player rolls against `baseDc + dcModifier`. That last one is a v13 prompt fix, still open, and cheap — the set has not been cut, so it needs no version bump.

**The decomposition that worked, worth reusing:** delegate by **file ownership, not by step**. `resolve/BASE.md` is touched by steps 2, 3, 4 and 5, so parallel executors split by step would have collided on it. The fences held with no collisions: E1 = `decide/BASE.md`, E2 = `resolve/BASE.md` (sole owner of all four steps' portions), E3 = the fourteen `resolve/*/{success,failure}.md` recipes, all three concurrent; then E4 = steps 6-7 (`src/` constants, `current_source/` resync, tests) alone and last, because its content-assertion tests depend on E2's and E3's edits having landed. The earlier 529 delegation blocker is long cleared.

**What the review pass caught, and the lesson in it (`5162d16`):** the three step-1-5 executors self-verified green and were all wrong about the same thing in different ways. `rest/success.md`'s inspiration grant had been nested inside the natural-20 clause — but rest **never rolls** (`classifier.ts:88` excludes it from `ROLL_ACTION_TYPES`, `PipelineActionStateMachine.ts:1160` only rolls when `needs_roll` is set, and `resolve/BASE.md` says outright that no crit applies to a pure rest), so that grant fired 0% of the time rather than rarely. Separately `combat/failure.md` was the one failure recipe whose non-stamina bullet lacked `Always:`, leaving a stamina-only failure legal, and the new test guarding that rule matched the file's engine-owned `modify_health` *prohibition* and so passed vacuously. **The general lesson: prompt prose that gates on an engine signal must be checked against the engine, not just read for sense** — a green suite proves nothing about whether a trigger is reachable. Worth applying to any future dial that keys off `D20:`, `needs_roll`, or a category flag.

**Verified state at `5162d16` (2026-08-01, lead, independently — not from a subagent report):** `npx tsc --noEmit` exits 0; `npx vitest run` is **89 files / 1662 tests** green (+5 over the 1657 baseline: four content assertions covering steps 1-5's prose, which had landed untested, plus the deliberate `loadPromptSet('v12')` attribution pin); `diff -r v13 current_source` is empty; `grep -rn "'v12'" src/` and `grep -rn v12 assets/prompts/decision-prompts/v13/` both come back empty. **v13 is now live in every path** — `PROMPT_SET_VERSION = 'v13'` and `ProdPipelineGateway` defaults to `loadPromptSet()`, so the next production action loads the new prose. The tree is no longer inert; the balance change is real and unmeasured.
