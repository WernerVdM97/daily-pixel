...
1. **Stage 4 — the v13 prompt set. Steps 0-5 are LANDED; steps 6-7 remain.** Per the plan's § Stage 4, routed via the `prompt-versioning` skill (read it from disk — `.claude/skills/prompt-versioning/SKILL.md`; it is not registered with the Skill tool, so invoking it by name fails). **The spec's "one atomic commit" did not survive a session boundary:** steps 1-5 landed as `272d56d` to make the work durable when the session ran out, so stage 4 will span two commits. Do not try to squash it — `272d56d` is pushed nowhere and rewriting it buys nothing. **What is done (`272d56d`):** step 0's 26-file copy is now tracked, and eleven of those files carry steps 1-5 — the DC ladder re-anchored to routine 11-13 / hard 16-18 / daunting 20-24 with the contract and field table widened to 10-24 (`decide/BASE.md`), the different-axis failure-cost rule in `resolve/BASE.md` rule 2 plus the five recipes reconciled to it (`rest`, `search`, `skill`, `travel`, and `other/failure.md` — which the spec wrongly listed as already compliant, since its flat OR list made a stamina-only failure legal), DC-scaled success reward, the inspiration dial (removed from `other/success.md` and `skill/success.md`, restricted in `rest/success.md`, demoted in BASE to rare-and-earned, nat-20 grant cut +2 → +1), `add_npc`'s optional `health` vocab, and the mercy branch in `resolve/combat/success.md` plus the `fatal blow` token documented in `resolve/BASE.md`. **Remaining, steps 6-7:** `PROMPT_SET_VERSION = 'v13'`; de-hardcode the four `'v12'` literals (`ProdPipelineGateway.ts:96` is the load-bearing one — it is the production default, so bumping the constant while it stays `'v12'` stamps v13 on v12 prose; plus `WorldEngineImpl.ts:626` and `:2090` and `DeepseekLlmGateway.ts:151`) and the stale doc comment at `ProdPipelineGateway.ts:54`; fix `v13/classify.md:1`, which still reads `# v12 · classify` (the two BASE headers are already v13); resync `current_source/` to be byte-identical to `v13/`; retarget the tests. `grep -rn "'v12'" src/` must come back empty. **CHANGELOG.md `[Unreleased]` has NOT been touched for stage 4** — required before any merge into `dev`.
   **Stage 4 has had NO review pass yet.** The three executors self-verified only; no `delegate-reviewer` has read the prompt diffs. Two things to put in front of that reviewer: (a) `skill/success.md` no longer contains `modify_rolls_remaining` at all, though the executor's report claimed it kept the nat-20 grant — confirm whether losing it there was intended or overshoot; (b) `rest/success.md`'s "exceptional night" was implemented as nat-20-gated, which is narrower than the spec's wording and may push the grant rate under the ~10% target. Surviving `modify_rolls_remaining` sites are now only `resolve/BASE.md` and `rest/success.md`.
   **Step 7 gotchas, already corrected in the plan (`8b6375b`):** `tests/llm/pipeline/stamping.test.ts` is a fourth pinned file the spec originally omitted — nine live `'v12/...'` stamp assertions plus `expect(PROMPT_SET_VERSION).toBe('v12')` at ~:74 — and the suite cannot go green without it. Conversely `tests/engine/action-mutations.test.ts`, `world-engine-impl.test.ts` and `pipeline-machine.test.ts` match `v12` only as a historical QA-era label in describe text and comments: **leave them alone.** Keep one deliberate test that `loadPromptSet('v12')` still loads, since historical rows stamped `v12/...` must stay attributable.
...
**The 529 delegation blocker is cleared** (2026-07-29 was `API Error: 529 Overloaded` on every `delegate-executor` spawn). The decomposition by file ownership worked as predicted on retry: three concurrent executors, fences E1 = `decide/BASE.md`, E2 = `resolve/BASE.md` (sole owner of all four steps' portions), E3 = the fourteen `resolve/*/{success,failure}.md` recipes, no collisions, all three self-verified typecheck clean and 89/1657 green. E4 (steps 6-7) never ran — it must run last, because the content-assertion tests depend on E2's and E3's edits having landed.

**Verified state at `272d56d` (2026-07-29, lead, independently — not from a subagent report):** `npx tsc --noEmit` exits 0; `npx vitest run` is **89 files / 1657 tests** green, unchanged, because nothing reads v13 until step 7 retargets the tests; `diff -rq v12 v13` shows exactly **11** differing files and no `v12` string survives anywhere in `v13/`. **The tree is inert in production** — `PROMPT_SET_VERSION` is still `'v12'` and `ProdPipelineGateway` still defaults to `loadPromptSet('v12')` literally, so no live path loads the new prose. That is why stopping here was safe.
