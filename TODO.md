...
## ⏭️ RESUME HERE - JSON seam arc: **M9 COMPLETE 2026-08-09 — all five slices closed; M10 (closeout) is the last milestone of the arc** (M8 COMPLETE 2026-08-06 + M8.5 smoke-run tooling COMPLETE 2026-08-06; M9 build plan lead-settled 2026-08-08; last touched 2026-08-09)

### ⚠️ START HERE — M10 is next, and it is no longer pure closeout (2026-08-09)

**Branch `feat/json-seam-protocol` is CLEAN at `7246d8c` and green (102 files / 2142 tests, typecheck clean).** M9 is done end to end: the Discord adapter is translate + paint over the protocol, and `commands/sleep.ts`'s `mapError` is the only runtime engine or controller import left anywhere in `src/discord/` — the DC-M9.8 exemption exactly.

**M10's original scope:** the interchangeability proof recorded (contract suite green against stub + real backend), the parent doc's gap-table "protocol seam" and "frontend adapters" rows settled, `CHANGELOG.md`/`TODO.md`/spec execution state updated, and the `AGENT_FORCE_FREE_ACTIONS` follow-up written up as the next arc's first item.

**Four items now ride with it, and the M9.4→M10 coordinator's steer is that they are not one kind of work.** Sequence the first on its own, ahead of the rest: (1) **the day-job leaf's catch paints through `webhook.editMessage` on an interaction nothing acked**, so a player likely sees "This interaction failed" over a correctly-painted error — a live player-facing bug, not an inherited gap, and it wants its own pass with its own review gate rather than a paperwork-level review. It probably needs the harness fixed FIRST: `webhook.editMessage` is mocked without the ack invariants `wireReplyAcks` enforces, so today the fault cannot be proven or regression-tested at all. Then the three mechanical ones: (2) deterministic real-backend corpus entries, still deferred on the SF3 same-weekday-class caveat; (3) `npm run typecheck` covers `src/**` only, so test-side drift stays invisible; (4) `src/render/map-render.ts` imports `SEPARATOR` and three direction helpers from `src/discord/format.js` — a render→discord dependency the M9.4 move created, where `format.ts` itself imports nothing and is pure string/emoji lookup, so the honest fix is rehoming it.

**One thing M10 must not let stand unstated, because M9's own gate overclaims it.** The gate reads "replay of the M8.5 corpus (stub + deterministic real-backend transcripts) byte-green", and the corpus contains only `stub-1d.protocol.json`. Every M9 closeout note's "replay green" therefore means the stub arm alone. This is recorded on the M9 milestone line in the spec, not hidden, but M10's interchangeability writeup is where it either gets discharged or gets stated as a permanent limit.

**The technique worth carrying, above any individual finding — it caught the biggest defect in both of the last two slices.** A check or transcript only proves something if it has been **shown to fail**. M9.3's lesson was that a snapshot written by the code under test is not evidence (`git worktree add <tmp> <pre-slice-sha>`, inject the same test, diff both sides). M9.4's was the same lesson one level up: the structural check was green, reviewed, and guarding three files out of a whole layer, and only a tamper found it. Tamper every gate you add, and record the tamper.
