...
1. **Release cut 0.3.3** then the doc loop, per the plan's § Release cut and § Doc loop. `poc-plus/release-a` still has to merge into `dev` first — nothing from stage 4 is on `dev` yet. The lead never merges `dev` → `main`; prompt the user.
2. **The cadence measurement** — the agent-player before/after run in § Doc loop. This is the only thing that can tell you whether RA-1 and RA-2 landed in band; see the overshoot note below, which the review sharpened considerably.
