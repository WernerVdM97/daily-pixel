...
## ⏭️ RESUME HERE - JSON seam arc: **M8.5 smoke-run tooling IN PROGRESS** (M8 COMPLETE: M8.0 screens oracle DONE + M8.1 crossing DONE, both review-clean; M8.5 checklist lead-settled 2026-08-06, last touched 2026-08-06)
...
**Next (M8.5 — smoke-run tooling, IN PROGRESS, checklist written 2026-08-06, stages 1-7 DONE 2026-08-06):** stages 1-7 of the 9-task checklist landed on `feat/json-seam-protocol` — protocol log (DC-S1, `beae128`+`d14db64`), session bootstrap (DC-S7, `c022d1f`+`8ae01c0`), observer boundary (DC-S4, `46e73b8`+`06434e6`), parity beats + realism arm (DC-S3, `4a8b3c9`+`162b108`), StubBackend extraction (`5606684`), stub-backed run (DC-S2, `c3f2362`+`cecece8`), replay runner + choice-fidelity (DC-S2+DC-S5, `28d5340`+`59d6fb3`) — 98 files / 2013 tests green, `npm run agent:replay` live-verified (stub run → 24 passed, 0 mismatches, exit 0). The verify-first probe settled the DC-S2 scope: real-backend byte-replay HOLDS for the deterministic class (characterId reproducible on a fresh engine; JSON-form deep-equality — the undefined-optional-field equivalence the contract suite's round-trip convention blesses), so the corpus can include deterministic real-backend transcripts (same-weekday-class caveat now covers the greeting AND the nightly tick — SF3). Next: **stage 8** — the DC-S5 contract suite (the "choice fidelity (agent stream)" describe with the recorded fixture through BOTH backends; generate the fixture IN-PROCESS at test time — the stage-9 smoke pattern — per the stage-7 coordinator pin; route the legality through replay.ts's validated machinery, don't reimplement; no fence widening), then **stage 9** — gate + docs (the protocol-transcript smoke assertion as a permanent test: in-process stub session → protocol log → replay → byte-equal; a stub transcript recorded into the M8.5 corpus for M9's replay gate — the corpus location is still an open stage-9 decision, no declared dir yet; changelog `[Unreleased]` → `### Internal` one-liner; spec-doc execution state + this TODO reconciled). M9 (the Discord rebuild) inherits the M8 byte net (screens oracle + contract suite) + the M8.5 replay/transcript tools as its drift net. Watch items carried: the D2 stale `/hi` resume edge stays unpinned (M9 territory), the charless `/help` slash arm now does one `getCharacter` read via `addCharacterFacts` (recorded note — M9's gate should know), the map focus flow-order pin stays indirect (adequate).
