...
## ⏭️ RESUME HERE - JSON seam arc: M5–M7 fully landed on `feat/json-seam-protocol` (M7.3 character creation done 2026-08-05); M8 (read-only screens) next — open with the screens oracle (last touched 2026-08-05)
...
**State.** Release A shipped as 0.3.3 (2026-08-02, merged via `dev`, tagged `v0.3.3`; `dev` at `56e127d`). The JSON seam arc runs on branch `feat/json-seam-protocol` (never commit/push/checkout `dev`/`main`). M5 (contract) + M6 (agent as protocol client) landed 2026-08-02; **M7 (bookends through the seam) COMPLETE 2026-08-05**: M7.0 bookend oracle (16 transcripts, `9e61011`, review clean), M7.1 rest+tick (`e669262` + `d47a835`), M7.2 `/hi` (`078aaba` + `8b08059`), **M7.3 character creation (`9814274` + `ed91227`)** — wizard state ownership settled as controller-held session state in `docs/decisions/wizard-session-ownership.md` (a genuine scoped extension of parent decision 1), `join.open` + `wizard.answer`/`wizard.choose`/`wizard.restart` + `character.create` cross the seam, a `wizard` ViewState variant + `wizardViewToDiscord` weld, `/join` is translate + paint (dead compat params + module-level `_router` until M9 — the dispatcher's join branch is frozen), and the agent's last engine-direct bookend left the harness (`seedCharacterViaProtocol`; `itemSetName` now REQUIRED for protocol seeding — the wizard's step 7 is mandatory; SEED fixtures gained `itemSetName` + lowercase alignment). The three M7.3 review items closed: `_userInFlight` double-click guard (oracle 18), wizard TTL-expiry (oracle 17), mock createCharacter non-persist (contract suite + live run assert the real path). D2 stale `/hi` resume edge LEFT RECORDED (M7.2 entry) — pinning is M9 territory. Gates: typecheck clean, 92 files / 1904 tests green, bookend oracle 19 transcripts + M1/M2 snapshots byte-green zero churn, `dispatchInteraction.ts` untouched. **M7 closeout live run (AGENT_DAYS=1, 2026-08-05): exit 0, 0 error / 0 warning findings, 3 outcomes + 1 night** — the M6-carried closeout caveat is CLOSED. `HEAD` `ed91227` is 18 commits ahead of `origin/feat/json-seam-protocol`. **Smoke-run tooling settled 2026-08-05** as DC-S1–S6 (spec doc § Smoke-run tooling plan): a dedicated **M8.5** slice between M8 and M9; M7.3 pinned the wizard-event names it will use.

**Next (M8 — read-only screens through the seam):** per the recorded M7.3→M8 coordinator steer (deepseek-v4-flash stand-in; kimi-k3 still suspended), **M8 MUST open with a screens oracle (M8.0) before any crossing** — the read-only screens have NO byte net today (the M1 oracle covers only `/stats`; the per-screen unit suites drive handlers directly, the same gap M7.0 closed for the bookends), and M9's byte-identical gate cannot see port drift without it. The steer's M8 checklist shape: ~12–16 additive transcripts over the six screens' slash + nav arms (incl. look's real null-location branch and the gate reroute); then one crossing slice (M8.1) for all screens; flat event names pinned for M8.5 (`screen.look`, `screen.map` `{ focus?: string }`, `screen.stats`, `screen.backpack`, `screen.journal`, `screen.help`); all screens ride `NoticeViewState` + `addCharacterFacts` (no new view variant, no beats, no controller stamps — the hi.open precedent); one controller method per screen (openLook/openMap/…); the "yet"→"first" NO_CHARACTER_COPY unification per screen (verify which charless nav edges are genuinely reachable); look's `resolveScene` becomes a controller constructor dep (the wizardDefs precedent), map composes in the controller layer importing the pure `map-render.ts`; the no-stamp property pinned with real spies (the M7.2 review-fix lesson). `help.ts` settled as `screen.help` (its Economy block carries game copy). Parity fence: M8 builds the events + reachable surface; DC-S3's parity beats are M8.5's. The M7.3 checklist + decisions record are in the spec doc; the M8 checklist is written when M8 starts.
...
- **Coordinator (kimi-k3) is unreachable as of 2026-08-05** — provider account suspended (insufficient balance). The recorded steers govern; the M7.3→M8 checkpoint ran on a **deepseek-v4-flash stand-in** (owner's call, 2026-08-05) — its steer is recorded in the spec doc's execution state. Resume kimi coordinator consults when billing is restored, or keep the stand-in.
