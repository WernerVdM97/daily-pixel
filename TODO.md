...
STATUS: [x] M4.0 DONE (commits 6d03890 code, b865fec closeout) — src/agent/viewToText.ts (discord.js-free, imports only view-state types): viewToText(view):string renders every ViewState variant to LLM-readable text (full variant, no embed-length degradation) and viewMoves(view):ViewMove[] enumerates the discrete buttons positionally (choice/bail/menu + the favoured insight hint surfaced for decision-2 parity). ViewMove is deliberately NARROWER than AgentMove (DA-6): it's view-derived button data; free-text/sleep are contextual harness-supplied moves. Tests tests/agent/viewToText.test.ts. Baseline now: 83 files / 1534 tests green.

NEXT — [ ] M4.1 (AgentPlayerGateway seam): in src/agent/, define the AgentPlayerGateway interface (chooseMove(input):Promise<AgentMove>), the AgentMove union (DA-6: {kind:'menu-pick';index} | {kind:'custom';text} | {kind:'choice';index} | {kind:'bail'} | {kind:'sleep'}), and the input shape (rendered screen text from viewToText + the viewMoves list + brief character state). Implement ProdAgentPlayerGateway reusing callDeepseek + a versioned prompt assets/prompts/agent-player/agent-v1.md + an AGENT_PLAYER_VERSION constant in src/llm/prompt-builder.ts (FOLLOW the prompt-versioning skill), recording one llm_calls row per call. Implement ScriptedAgentPlayerGateway (deterministic, move-list driven, must never throw) for tests. Unit-test the stub + the real impl's response PARSE against a canned JSON body via injected fetch — no network. Model the whole thing on the ProdPipelineLlmGateway (real) / PipelineScriptedGateway (stub) pair (src/llm/pipeline/, src/sim/PipelineScriptedGateway.ts). THEN: [ ] M4.2 harness bring-up + one action end-to-end (src/agent/harness.ts: build the engine above, a SessionController, seed a character engine-direct, drive ONE action through the controller with the brain — openActionMenu→viewToText→chooseMove→beginDayJob/commuteForWork/runWork or beginCustomAction/runCustomAction→the decision loop beginChoice/resolveChoice/stepChoice until outcome — emit a transcript; test with scripted brain + scripted pipeline gateway, fully deterministic). [ ] M4.3 full-day + multi-day loop (loop until rollsRemaining=0, then the sleep move→restAtOak/tick→advanceDays→next day). [ ] M4.4 QA capture (wrap every call so exceptions/dead-ends/illegal-move-loops/invariant breaches (neg HP/stamina/wealth, rolls underflow) become structured transcript findings + a run summary; a throwing run still writes a repro). [ ] M4.5 feedback pass (a critic LLM reads a completed transcript → qualitative playtest report: pacing/clarity/fun/difficulty). [ ] M4.6 closeout (verify src/agent/ imported by neither discord/ nor sim/; npm test green with stubs; document a real-LLM smoke run; CHANGELOG [Unreleased]; tick M4 in the parent doc).

DISCIPLINE (every slice): home = src/agent/ (peer to discord/ and sim/, imported by neither; opt-in `npm run agent:play`, never in npm test); one slice → code commit + docs closeout tick; run a fresh-context adversarial review per slice (delegate-reviewer) since this is new code — judge on faithfulness/correctness, not byte-identity; SCOPE FENCE: no src/discord/ changes, no new controller methods for the bookends (DA-4), no formal JSON protocol/event-router, sim/ unchanged, the real LLM never runs in npm test — engine/controller changes only if a genuine seam gap blocks a slice (flag as a decision, don't smear flow into the harness). Use orchestrated-delegation (lead specs+verifies+commits; delegate-executor implements to spec; delegate-reviewer adversarially checks; delegate-fixer applies accepted findings). NEVER commit/push/checkout main or dev.
