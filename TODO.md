...
## ⏭️ RESUME HERE — the JSON seam arc is **COMPLETE** (M5–M10 all landed 2026-08-09). Next task: `AGENT_FORCE_FREE_ACTIONS`.

### ⚠️ START HERE — the arc is closed; the branch is ready for the owner to merge

**Branch `feat/json-seam-protocol` is CLEAN at `252b8d2` and green: 102 files / 2150 tests, both typecheck projects clean, both protocol-corpus arms replay byte-green.** Nothing is in flight. The owner merges to `dev` (agents never do).

**What M10 closed, beyond the paperwork it was originally scoped as.** M10.0 repaired a live player-facing fault: the day-job button's catch painted through `webhook.editMessage` on an interaction nothing had acked, so on a pre-beat failure the player got **nothing painted at all** plus Discord's "This interaction failed" — worse than the handover described, because the leaf's own `.catch(() => {})` swallowed the rejection. M10.1 split `format.ts` (half of it was genuinely Discord transport, so the spec's "pure relocation" was wrong), made the layer check run in **both** directions, brought the test suite under typecheck, and pinned the recording clock so the real-backend corpus arm M9's gate always claimed finally exists.

**The one thing to carry forward, because it caught the biggest defect in each of the last four slices.** A check only proves something if it has been shown to fail — and M10 found the sharper version: **a check can be non-vacuous, red for the right reason, and still be watching the wrong variable.** M10.0's harness invariant was proven by tamper, went red at exactly the right test, and the fix it drove was *still wrong*, because `beatPaint`'s truthiness and "this interaction is acked" are different propositions that agree on every path a test happened to cover. Only a fresh-context reviewer reading the assignment as synchronous caught it. Tamper every gate, then check the gate and the fix are reading the same fact.

**M10.1's variant of the same lesson, which cost the most time:** two things that must agree will not, unless something forces them to. The clock pin was specced "record + replay" and built replay-only; the corpus recorder and the replayer established different boot state; a test helper and the module it duplicated drifted the moment one changed. **None of those threw** — all three surfaced as unexplained byte differences, and each was fixed by collapsing two definitions into one rather than teaching them to match.

### ▶️ NEXT TASK — `AGENT_FORCE_FREE_ACTIONS` (the next arc's first item, owner-settled)

The agent-player **cannot validate balance today**, and this blocks two RA residuals that have been unverified since Release A. The brain overwhelmingly picks the day-job menu over free actions, and `stripWorkInspiration` strips inspiration from work actions by design, so both v12 and v13 returned a **structurally** 0% grant rate — a measurement artefact, not a finding. RA-2's ~10% target and 3.2–3.7 band stay unverified until either human play or a harness switch that forces free actions. The realism arm (M8.5) did **not** fix move variety; it was tried and the brain still funnelled into `menu-pick 0` / `choice 0`.

Scope when it starts: a switch (or a brain-prompt ration) that makes free actions reachable in a run, then a measurement pass. Note the harness is otherwise a good QA/crash instrument — this is about what it can *measure*, not whether it works.

### Limits the arc leaves behind (state these rather than re-discovering them)

- **The transport is in-process throughout** (parent decision 3). Nothing has crossed a wire, a process boundary or a network, so "the protocol shape is the asset" is a design claim, not a demonstrated one.
- **The structural checks constrain imports and named calls, not dataflow.** `controller` passed as an argument and re-bound under another name inside a helper is real indirection the source-regex checks do not close. Stated in the test, not implied.
- **The real-backend corpus arm is deterministic because its LLM is scripted and its d20 fixed.** It proves the seam's determinism, not the engine's behaviour under a live model.
- **`commands/sleep.ts`'s `mapError` is the one runtime engine import left in `src/discord/`** — the DC-M9.8 admin-tick exemption, deliberate and named in the check itself.
- **Live agent runs need `set -a; . ./.env; set +a`** (no dotenv) and cost real tokens; keep `AGENT_DAYS` small. The deterministic gate is the correctness proof.

**Read first if picking the arc's history up:** [`docs/engine/json-seam-protocol.md`](./docs/engine/json-seam-protocol.md) (§ Execution state tail carries the through-line and the limits above). Parent: [[layer-boundaries-and-json-seam]] — its "protocol seam" and "frontend adapters" gap-table rows are now settled. The M0–M4 slice pattern: [[json-seam-build-plans]].

### Arc history (kept for the record)

**M9.3 (the dispatcher rewrite) COMPLETE 2026-08-09** — nine commits, judge BLOCK on the first M9.3.2c attempt (reworked, not replayed), then a fresh-context review verdict of BLOCK with one blocker and five should-fixes, all accepted and fixed. **381 snapshot insertions, 0 deletions** against the pre-slice baseline across three oracles; the screens oracle and the M8.5 corpus byte-untouched; replay green. Six churn classes, all declared and all absorbed inside the slice. `CommandHandler` gained an optional `onNav` second parameter — assignment-compatible, so zero assertion churn — and the four nav-weld sites pass a function-local closure, which is what DC-M9.6's widened-`getNavButtons` premise could never have delivered (the sites are handlers returning `Promise<string>`; they discard the fact). Three router arms had to start carrying `nav` because the read they replace was outcome-independent: the two slash feedback surfaces, the two `rest.begin` guard rejections, and **every `internal` envelope** — a fault screen kept its nav bar before the port, and the naive port stripped the buttons off every screen the seam absorbs a fault into. The review's blocker was the mirror image: `internal` has two sources, and `openActionMenu`'s `resume-error` arm IS the ordinary 30-minute timeout, so paging `notifyAdmin` on the collapsed code woke an operator every time a player walked away from a decision screen (M9.2's blocker 1, one leaf over). `facts.internalFault` now separates the thrown kind from the mapped kind, following `persistFailed`. Three behaviour changes are recorded, beyond the owner-signed profanity move: the custom-modal leaf gains the roll guard M9.2 left it (pre-port, a 0-roll player clicking Custom… got a free unpaid action, LLM call and loot included), its charless arm replies once instead of deferring first, and the generic nav leaf's read-order change moves the D2 bar. Full detail: `docs/engine/json-seam-protocol.md` § M9.3 + § Execution state tail.
...
- [>] **Give the agent-player a way to exercise the quest loop** — an `AGENT_FORCE_FREE_ACTIONS`-style switch, or a brain prompt that rations day-job picks. Without it the harness can't answer any balance question about free actions. **Promoted 2026-08-09 to the next arc's first task — see § NEXT TASK at the top.**
