...
HANDOVER (M3.6/M3.7). Parent spec docs/engine/layer-boundaries-and-json-seam.md; stage plan docs/engine/json-seam-build-plans.md (M3 section — per-slice DC checklists + execution state live there). Branch feat/m3-controller-extraction, last commit 28baae1. M3.0–M3.5 DONE: the SessionController (src/controller/SessionController.ts, never imports discord.js) now owns feedback/bug submits, action:choice/bail (engine.resolvePendingChoice killed the pendingDecisions map + getChoiceLabel + setPendingDecision + handleActionChoice + applyActionResult), the day-job menu (nav:action + slash /action via composeActionMenu/MenuViewState), the day-job work flow (action:dayjob: via beginDayJob/commuteForWork/runWork), and the custom-action modal submit (action:custom:modal via beginCustomAction/runCustomAction + the shared renderStartResult). View-states live in src/view/viewState.ts (Decision/Outcome/Notice/Menu/Loading/Commute); the medium step is src/discord/viewToDiscord.ts (*ViewToDiscord); the discord.js-free view-builders are src/view/actionViewState.ts; the day-job domain (getDayJobActions/getWorkplaceLocation/buildActionHints/CID_DAYJOB*) is src/controller/dayJob.ts. RECONCILE FIRST before building: M3 checkboxes vs git log, typecheck + npm test green (82 files / 1519 tests, ZERO snapshot churn) — a snapshot change means the port drifted, fix the port never re-bless. NEXT — M3.6: port the generic nav: route + nav:sleep loading beat + the slash-arm flow bits (character-gate reroute, updateLastPlayed stamping) in src/discord/dispatchInteraction.ts; the ephemeral command-list policy + navResponseMode + buildComponentPayload + the error funnel (notifyAdmin/safeErrorReply, parent decision 5) STAY adapter (Bucket A). Then M3.7 closeout: confirm the dispatcher is transport-only + pendingDecisions gone, oracle (tests/discord/dispatch-oracle.test.ts) + M2 snapshots green, add a CHANGELOG [Unreleased] entry, tick M3 in the parent doc. Established seam (reuse it): adapter extracts transport → controller returns a ViewState (+ semantic facts for broadcast/announceCollapse) → adapter paints via *ViewToDiscord; loading/notice micro-embeds that aren't yet view-states stay inline in the adapter. Use orchestrated-delegation (lead specs + verifies + commits between slices; delegate-executor implements to spec; delegate-reviewer adversarially checks behavioural slices; delegate-fixer applies accepted findings). NEVER commit/push/checkout main or dev.
