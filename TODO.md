...
**State.** Release A is fully built, P1 (the daunting-band restatement) included, and the release is cut: `VERSION`/`package.json` at `0.3.3`, `CHANGELOG.md` `[0.3.3]` promoted and dated 2026-08-02, `assets/release-notes/v0.3.3.yml` written. Baseline **89 files / 1664 tests**, typecheck clean (the cut itself added one: the asset-schema sweep generates a test per file in `assets/release-notes/`). **v13 is live in every path**, `PROMPT_SET_VERSION = 'v13'` and `ProdPipelineGateway` defaults to `loadPromptSet()`, so production loads the new prose and the balance change is real.
...
- [ ] **RESOLVE is told to scale the reward by the DC attempted, and is never sent a DC** *(found by P1's review, deferred out of its prompt-only fence)* — `resolve/BASE.md` and `resolve/combat/success.md` both key reward size to the difficulty attempted, but `buildResolveUserMessage` (`src/llm/pipeline/pipeline-messages.ts:63-83`) sends the verdict, the d20, the action type, the chosen label and stat, the fatal-blow token and the scene body, and no DC of any kind. So RA-1's reward half runs on inference from the option label, and `combat/success.md` names a `baseDc` it cannot read. The fix is the shape stage 2 already used for `fatalBlow`: one optional field on both resolve inputs, emitted as a bare token, then read by the recipes. Worth doing early in the next prompt set, since it pays the player for the gamble P1 just made reachable.
