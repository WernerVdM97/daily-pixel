...
HANDOVER — M4 (agent-player adapter). Read FIRST: the full M4 plan (goal, scoping calls, design calls DA-1..6, slice sequence, scope fence) lives in docs/engine/json-seam-build-plans.md under `## M4`; the parent spec is docs/engine/layer-boundaries-and-json-seam.md (milestone checkboxes + settled decisions 1-6; changing a decision needs a decisions/ record). Branch feat/m4-agent-player off dev (bd8db8b); M3.0–M3.7 are DONE and merged to dev. RECONCILE FIRST every resume: M4 checkboxes vs git log, `npm run typecheck` + `npm test` green.

WHAT M4 IS: an in-process, Discord-free "agent-player" — a peer adapter over the M3 seam — that stands up a real WorldEngine + SessionController and plays the game with a real LLM brain, all features on, for BOTH bug-hunting/QA and playtest feedback. Distinct from sim/ (engine-level, LLM-cut, months of data); the agent-player enters at the controller with the real LLM. Settled forks: seam = call SessionController's typed methods directly in-process (no JSON event-router; Discord adapter NOT rebuilt); brain = real DeepSeek from the first playing slice, with a deterministic scripted stub in tests/CI (never a live LLM call in npm test).

CONSTRUCTION RECIPE (from 2026-07-20 recon, all file:line in the plan doc): (1) Engine = prod-faithful WorldEngineImpl on initDb(':memory:')+migrate (seeds SEEDED_LOCATIONS/EDGES), five repos, REAL assets via loadCharCreationAssets()+buildDayJobIncomeMap() (index.ts:172-189) so createCharacter derives real stats, and a REAL ProdPipelineLlmGateway({apiKey,model}) as pipelineLlmGateway (src/llm/pipeline/ProdPipelineGateway.ts:80) — NOT PipelineSimEngine, which never ticks. Nightly advance via advanceDays(engine,1) (src/sim/time.ts). (2) The LLM here is DeepSeek via callDeepseek (src/llm/deepseek-transport.ts:44), NOT Anthropic; reuse it verbatim (audit-in-finally, throw-loud, JSON-mode). (3) Three bookends are Discord-only with NO controller seam — character creation (join wizard), rest+nightly-tick (engine.restAtOak / engine.tick(true)), and /hi — so the agent calls the ENGINE DIRECTLY for these (like sim/ does); extending the controller to own them is OUT of M4 scope (would re-open the just-settled M3 surface).

STATUS (baseline now: typecheck clean, 85 files / 1553 tests green): [x] M4.0 (6d03890 + b865fec) src/agent/viewToText.ts: viewToText + viewMoves. [x] M4.1 (a2be63a) src/agent/ AgentPlayerGateway seam: the interface + AgentMove union (DA-6) + ChooseMoveInput/LegalMove/AgentCharView input shape; ProdAgentPlayerGateway (reuses callDeepseek, throw-loud + record-in-finally, mirrors ProdPipelineGateway) and ScriptedAgentPlayerGateway (deterministic move-list, fails loud on exhaustion); versioned prompt assets/prompts/agent-player/agent-v1.md + current_source.md mirror; AGENT_PLAYER_VERSION + loadAgentPrompt in src/agent/agentPrompt.ts. The brain picks by list-position index (LLM contract `{thought,choice,text?}`), the gateway maps back to the stored AgentMove so menu-pick/choice carry the VIEW button index, not the list position. Tests tests/agent/agent-gateway.test.ts (15). [x] M4.2 (cce05ca) src/agent/engineHarness.ts buildAgentEngine (DA-1: :memory:+migrate, five repos, real assets + real scenes -> faithful getCurrentScene; pluggable pipeline gateway - injected scripted for tests, real ProdPipelineLlmGateway from apiKey on a live run) + src/agent/harness.ts AgentHarness (seedCharacter engine-direct DA-4; playOneAction drives stampLastPlayed -> openActionMenu -> viewToText/viewMoves -> chooseMove -> day-job or custom -> decision loop until outcome; emits a Transcript) + src/agent/agentMoves.ts (view buttons -> LegalMove via customId, + contextual sleep/custom, isLegal guard) + src/agent/transcript.ts. Tests tests/agent/harness.test.ts (4, deterministic: scripted brain + PipelineScriptedGateway + fixed d20).

OPEN DECISION for the user (M4.1): AGENT_PLAYER_VERSION lives in src/agent/agentPrompt.ts, NOT src/llm/prompt-builder.ts where the prompt-versioning skill's table places version constants. Rationale: DA-5 isolation (core LLM code must not depend on the opt-in QA adapter); every prompt-versioning RULE is honoured (own folder, versioned file, current_source.md mirror, independent constant, rows stamped agent-v1). If the user prefers it in prompt-builder.ts for skill-table consistency, it is a small follow-up move.

NEXT — [ ] M4.3 full-day + multi-day loop. RECON DONE: restAtOak(userId) (returns the character to the Oak, records the rest) and tick(true) (advances one game day: refills rolls, regens, pays income) are BOTH on the WorldEngine interface, so the DA-4 end-of-day bookend is a clean two-call engine-direct sequence (no need for src/sim/time.ts advanceDays or the WorldEngineImpl concrete type). DESIGN (was mid-implementation, reverted to land cleanly at M4.2): add to AgentHarness - playDay(): loop playOneAction until slept/no-rolls/no-character, with a STUCK_LIMIT (~5 consecutive non-outcome actions -> log a finding, end day) and a MAX_ACTIONS_PER_DAY (~50) hard cap so a stuck brain can't spin; returns a DaySummary {dayNumber, outcomes, ended:'slept'|'no-rolls'|'stalled'|'no-character'}. playDays(n): for each day run playDay, break on no-character (fatal), else endDay() = restAtOak(userId) then tick(true) then transcript.day(newDay). Also add src/agent/play.ts + a package.json "agent:play":"tsx src/agent/play.ts" script (DA-5 entry, opt-in real run gated on DEEPSEEK_API_KEY: buildAgentEngine({apiKey,model,recordLlmCalls:true}) + ProdAgentPlayerGateway({apiKey,model,recorder:new LlmCallRepository(ae.db),verbose:true}) + createAgentHarness, seed a character, playDays(N), print the transcript). TEST (scripted brain + scripted pipeline): a multi-day sleep run (day_number advances, rolls refill after tick) and a full-day no-rolls run (brain works `rollsRemaining` actions then openActionMenu returns no-rolls without asking the brain again; each action = 3 brain calls with the goblin script: custom/menu-pick + 2 decision picks). THEN: [ ] M4.4 QA capture (wrap every controller/engine call so exceptions/dead-ends/illegal-move-loops/invariant breaches (neg HP/stamina/wealth, rolls underflow) become structured transcript findings + a run summary; a throwing run still writes a repro up to the failure). ALSO fold in the two M4.2 review deferrals: (a) a commute-beat transcript event in doDayJob (viewToText already renders the commute screen), (b) broaden test coverage of openActionMenu no-rolls/resume-* arms and beginDayJob invalid-job/unsafe arms. [ ] M4.5 feedback pass (a critic LLM reads a completed transcript -> qualitative playtest report: pacing/clarity/fun/difficulty; reuse the critic-gateway pattern if it fits, else a second agent-player prompt family). [ ] M4.6 closeout (verify src/agent/ imported by neither discord/ nor sim/; npm test green with stubs; document a real-LLM smoke run; CHANGELOG [Unreleased]; tick M4 in the parent doc + parent spec).

DISCIPLINE (every slice): home = src/agent/ (peer to discord/ and sim/, imported by neither; opt-in `npm run agent:play`, never in npm test); one slice -> code commit + docs closeout tick + a fresh-context adversarial review (delegate-reviewer, since this is new code - judge on faithfulness/correctness, not byte-identity; act on findings before committing). Established review cadence has been productive: M4.1 found a coverage gap (fixed), M4.2 found one CONFIRMED bug (immediate-outcome path transcripted viewPublic instead of viewPrivate - fixed + locked by a test). SCOPE FENCE: no src/discord/ changes, no new controller methods for the bookends (DA-4), no formal JSON protocol/event-router, sim/ unchanged, the real LLM never runs in npm test - engine/controller changes only if a genuine seam gap blocks a slice (flag as a decision, don't smear flow into the harness). NEVER commit/push/checkout main or dev. NOTE: agent TESTS may import src/sim/PipelineScriptedGateway (the deterministic pipeline stub) - that is test-only and does not violate the src/agent/ SOURCE isolation.
