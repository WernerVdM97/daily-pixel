...
- [ ] **`hiScreen.isWeekend()` reads the LOCAL weekday while the tick reads UTC**, so they can disagree near midnight. Panels avoid it by pinning the start at noon UTC.
