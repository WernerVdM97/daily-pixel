...
**Next (M8.5 — smoke-run tooling, IN PROGRESS, checklist written 2026-08-06):** per the spec doc § Smoke-run tooling plan (DC-S1–S6, lead-settled 2026-08-05) + **DC-S7 (owner requirement 2026-08-06 — session spawn modes): every agent session spawns as a FRESH player through the full `/join` wizard walk on a per-session fake id, or INHERITS an existing PC by its recorded userId (AGENT_INHERIT=1 + AGENT_USER_ID; the engine has no name→userId lookup, so the inherit handle is the recorded fake id; names are not unique in the DB)**. The full M8.5 task checklist (9 tasks, event names pinned) is in the spec doc — first task is the protocol log (DC-S1) incl. the recording-gap fix (the creation walk must flow through the harness's recorded dispatch, today it dispatches through the raw router in `play.ts` and is NOT recorded). M9 (the Discord rebuild) inherits the M8 byte net (screens oracle + contract suite) + the M8.5 replay/transcript tools as its drift net. Watch items carried: the D2 stale `/hi` resume edge stays unpinned (M9 territory), the charless `/help` slash arm now does one `getCharacter` read via `addCharacterFacts` (recorded note — M9's gate should know), the map focus flow-order pin stays indirect (adequate).
