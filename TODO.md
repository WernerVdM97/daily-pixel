...
## ✅ JSON seam arc — CLOSED 2026-08-09

M0–M10 all shipped and merged to `dev` (`db2d751`). Design doc stays live at [[layer-boundaries-and-json-seam]]; the build plans, handovers and execution state are archived under `docs/archived/json-seam/` if you need the rationale or the milestone record. Two limits it leaves behind, worth not re-discovering: the transport is **in-process throughout**, so nothing has crossed a wire; and the structural checks constrain imports and named calls, **not dataflow**.

## scratchpad (humans start here)

### Prod-data review — two-week window (2026-07-19 → 08-02, snapshot `warden-20260802-212213`)

Day 27; 37 actions over 8 active days, 3 active players, **2 of 5 characters churned after week one**. The only feedback filed all window was "This sucked" (F#14, 12s after bailing a fight) — players file only when something breaks. The 08-02 wolf fight is the pain centre (B#17-19): 13 rounds, ~20 min wall-clock, ending in a bail worth −1 stamina, with visible state desync (HP bar bouncing, difficulty flipping hard→medium, a minion card for a wolf). It was also the first human exposure to the v13 16-18 ladder. Decide latency roughly tripled over the last 3 days (avg ~19s on 08-02, 10 calls >30s) — provider contention suspected, since the spike predates v13. Health checks green: parse_ok 99.8%, zero fallbacks, zero validation warnings, zero HTTP errors.
...
### POC+ Release A — residuals still open (shipped 2026-08-02 as v0.3.3)

Release A itself is done; what survives here is only the work it left **unverified**. Full write-ups: [[poc-plus-roadmap]] § Re-sequencing, [[resolve-difficulty-signal]].
