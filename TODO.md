...
## ⏭️ RESUME HERE - JSON seam arc: **M8 COMPLETE** (M8.0 screens oracle DONE + M8.1 crossing DONE, both review-clean); **M8.5 smoke-run tooling is next** — its checklist is written when it starts (last touched 2026-08-06)
...
**Next (M8.5 — smoke-run tooling, the M8/M9 gap slice):** per the spec doc § Smoke-run tooling plan (DC-S1–S6, lead-settled 2026-08-05), one dedicated slice between M8 and M9: protocol transcript (DC-S1), the `AgentObserver` typed boundary + structural pin (DC-S4), replay + stub-backed run (DC-S2), choice-fidelity invariants (DC-S5), scripted parity beats on the now-landed `screen.*` events + `hi.open` + the wizard walk (DC-S3), gates (typecheck + suite + a protocol-transcript smoke assertion). **The M8.5 task checklist is the fresh session's first task** (write-checklists-as-you-go — it pins the M7.3 wizard-event and M8 `screen.*` event names the outline deliberately leaves generic). M9 (the Discord rebuild) inherits the M8 byte net (screens oracle + contract suite) + the M8.5 replay/transcript tools as its drift net. Watch items carried: the D2 stale `/hi` resume edge stays unpinned (M9 territory), the charless `/help` slash arm now does one `getCharacter` read via `addCharacterFacts` (recorded note — M9's gate should know), the map focus flow-order pin stays indirect (adequate).
