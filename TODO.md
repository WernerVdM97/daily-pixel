...
## ⏭️ RESUME HERE - JSON seam arc: M8.0 screens oracle DONE (18 transcripts); M8.1 the crossing slice is next — checklist written when it starts (last touched 2026-08-06)
...
**State.** The JSON seam arc runs on branch `feat/json-seam-protocol` (never commit/push/checkout `dev`/`main`; `HEAD` `ceda557`, 20 commits ahead of `origin/feat/json-seam-protocol`). M5 (contract) + M6 (agent as protocol client) landed 2026-08-02; **M7 (bookends through the seam) COMPLETE 2026-08-05** — M7.3 character creation (`ed91227`) closed its three review items and settled wizard state ownership in `docs/decisions/wizard-session-ownership.md`; `itemSetName` is now REQUIRED for protocol seeding. **M8.0 (screens oracle) DONE 2026-08-06 (`ceda557`, review clean)** — 18 golden transcripts over the six screens' slash + nav arms (look 5 incl. null-location + gate reroute; map 3 incl. focus drill-down; stats 2 nav-only; backpack 3; journal 3; help 2 incl. the charless no-gate pin), test-only, zero `src/` edits; 93 files / 1922 tests green, oracles + snapshots zero churn. Smoke-run tooling settled as DC-S1–S6 (spec doc § Smoke-run tooling plan): a dedicated **M8.5** slice between M8 and M9.

**Next (M8 — read-only screens through the seam):** per the recorded M7.3→M8 coordinator steer, **M8 MUST open with a screens oracle (M8.0) before any crossing** — the read-only screens have NO byte net today (the M1 oracle covers only `/stats`; the per-screen unit suites drive handlers directly, the same gap M7.0 closed for the bookends), and M9's byte-identical gate cannot see port drift without it. The steer's M8 checklist shape: ~12–16 additive transcripts over the six screens' slash + nav arms (incl. look's real null-location branch and the gate reroute); then one crossing slice (M8.1) for all screens; flat event names pinned for M8.5 (`screen.look`, `screen.map` `{ focus?: string }`, `screen.stats`, `screen.backpack`, `screen.journal`, `screen.help`); all screens ride `NoticeViewState` + `addCharacterFacts` (no new view variant, no beats, no controller stamps — the hi.open precedent); one controller method per screen (openLook/openMap/…); the "yet"→"first" NO_CHARACTER_COPY unification per screen (verify which charless nav edges are genuinely reachable); look's `resolveScene` becomes a controller constructor dep (the wizardDefs precedent), map composes in the controller layer importing the pure `map-render.ts`; the no-stamp property pinned with real spies (the M7.2 review-fix lesson). `help.ts` settled as `screen.help` (its Economy block carries game copy). Parity fence: M8 builds the events + reachable surface; DC-S3's parity beats are M8.5's. **The M8 build plan is in the spec doc (lead-settled 2026-08-06): M8.0 (18 transcripts, fully specced) is DONE — review clean, 3 nits triaged (1 residual recorded: the mock logs no `getExits`/`getDiscoveredGraph` calls, M8.1's home), the M8.0→M8.1 coordinator steer recorded (three M8.1 checklist obligations). M8.1 — one crossing slice for all screens — is NEXT: six `screen.*` events (`screen.look`/`screen.map { focus? }`/`screen.stats`/`screen.backpack`/`screen.journal`/`screen.help`), `NoticeViewState` + `addCharacterFacts`, controller `open*` methods, `resolveScene` constructor dep (harness keeps its fixed stub), no-stamp spies, contract suite in the same commit; its checklist is the fresh session's first task. The M8.1 gate reads: the "yet"→"first" unification churns EXACTLY the five charless-nav snapshots (transcripts 4/8/10/13/16), nothing else.**
