...
- **The user-facing LLM error strings follow the provider**: the `OpenRouter API error` / `OpenRouter returned empty response` / `failed to parse OpenRouter response` prefixes are what `ErrorMapper` matches to turn a stage failure into "The warden's vision is clouded", so leaving them reading `DeepSeek` would have quietly degraded every LLM failure to the generic fallback message.
