...
- **`/help` stops teaching the wrong roll count and wizard length**: the command list said "2 rolls per day" (the allowance has been 3 since `0.2.2`) and "6-step wizard" (the wizard's own footer counts seven), so the one screen a new player trusts was the one screen still wrong. Copy fixed in `src/controller/helpScreen.ts`; the screens oracle and `help.test.ts` re-pinned.
...
- **`docs/CONVENTIONS.md`'s marker table teaches the list-item form and closes the set**: the markers were listed bare, so the vault drifted into a bare `[!]` / `- [!]` split (bare is prose to Obsidian, not a list item) and invented letters (`[P]`, `[M]`) slipped in. § List markers now shows `- [!]` and carries an NB: a new marker letter is added to the table before the doc that uses it lands.
