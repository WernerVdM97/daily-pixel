...
- **An action that auto-resolves before any decision still shows its opening frame** — travel and rest resolve inside `start()` with no decision beat, so the frame (`renderOpeningFrame`, one register per CLASSIFY type) had nowhere to render and those outcomes carried the bare location scene alone. The outcome screen now draws it ahead of the scene, from the pre-action slots only: the PC's name/HP and, for travel, the origin location the character has already left (the destination stays the wireframe's literal `????`). Both out-of-band start paths are covered, `/action <text>` and the day job, since both funnel through `renderStartResult`; combat is untouched, its outcome already drawing its own frame. `/sleep`'s own rest screen is not an action and never passes through CLASSIFY, so it stays frameless.
