...
- **`scripts/send-dm.ts` can carry an embed (`--embed <file>`)** — a JSON embed, or an array of them, sent with the body or on its own, because a plain body has a single 2000-character budget for everything a message carries: one that must hold per-item cards otherwise loses either the items or the links. The embeds are validated against Discord's limits (field name 256, value 1024, 25 fields, description 4096, 6000 across the message), so an over-long field is refused by name and count the way an over-long body already was.
...
- **Meta-oil's digest becomes an index plus proposal cards** — one message in two parts: content under 900 characters carrying the signals, the window and spend, what is still open and the reaction legend, and one embed carrying a card per proposal with its six labelled fields (`signal`, `why`, `files`, `diff`, `verify`, `blast`). Every issue, PR and file named is an absolute link, `.pi/factory/memory/` is never linked because it is gitignored and the link would 404, and the full drafted edit is written to `meta/proposals/<date>.md` before the card goes out, since an approval is applied days later in a fresh context. Scrumo stays on plain content: three actions need no cards.
