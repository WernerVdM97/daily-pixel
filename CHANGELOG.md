...
- **Every DeepSeek agent pin moves onto OpenRouter, held to the DeepSeek host**: the seven factory and delegation agents on the direct API (`delegate-{coordinator,executor,reviewer,fixer}`, `factory-scrumo`, `factory-triage`, `meta-oil`) now pin `openrouter/deepseek/deepseek-v4.1-flash`, and the job ledger's stage wrapper (`WRAPPER_MODEL` in `scripts/factory-jobs.ts`) follows. The route is not left to OpenRouter's load balancer: the `only: ["deepseek"]` / `allow_fallbacks: false` override on that model in `~/.pi/agent/models.json` fixes the serving host to DeepSeek itself, because a session that switches host gets a cold prompt prefix every time and the cache-read rate is what makes a long agent run affordable. This reverses the previous "direct DeepSeek only, never via OpenRouter" owner rule. Two things the move had to preserve: the OpenRouter entry takes text **and image**, so triage's image ingestion survives, and its thinking map is `off`/`high`/`xhigh` with no `max`, so `meta-oil` and `factory-triage` moved from `max` to `xhigh` rather than being silently clamped (the direct provider's map is the mirror image: `low`/`high`/`max`, no `xhigh`). GLM pins are untouched, so the judge and the escalator stay a different model from the work they judge. Verified on a throwaway session: the pin resolves to provider `openrouter`, and OpenRouter reports the generation as served by `DeepSeek`.
