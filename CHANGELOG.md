...
- **The loops move off DeepSeek's peak hours, and the retired model aliases go**: triage shifts to 07:30/19:30 local and the executor to 06:00, which puts every deepseek call outside the provider's 01:00-04:00 and 06:00-10:00 UTC peak windows, where the same tokens cost half as much. Every deepseek pin moves off the retired `deepseek-v4-flash` and `deepseek-v4-flash-vision-exp` names onto `deepseek-flash`, which is the model those requests were already being served by and billed as (DeepSeek V4.1 Flash), and triage's `xhigh` becomes the `max` it was already being clamped up to, since the direct provider carries no `xhigh` tier. All seven schedules are **enabled**, so the cadence table in `docs/engine/dark-factory.md` is now the live one rather than a plan for when they resume.
