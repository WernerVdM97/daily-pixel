...
## [Unreleased]
### Added
- **Markdown LLM input + coherence critic (decision prompt v9)** — the decision context sent to the LLM is now a **markdown briefing** it reads as a scene rather than a `key=value`/embedded-JSON dump: a `## You` block with a pre-joined `Score + Gear = Bonus` ability-checks table (no arithmetic left for the model), a structured `### Inventory`, a `## Scene` safety tag (`safe`/`unsafe`), split NPC/other-player lists, a `### Story so far`, `### Known locations`, and the player's input fenced as a blockquote last; the Warden lore is lifted out of the NPC list into a fenced out-of-character GM note. Response JSON contract is unchanged. Adds a **coherence critic** (**on by default**; opt out with `ENABLE_COHERENCE_CRITIC=false`): a second LLM pass that reviews each authored beat against engine truth (roll verdict, final mutations, context, validator warnings) and rewrites only the *prose* to match — never mutations/DC/rolls — patching a minor defect or triggering one bounded re-decide, and failing open so it can never block gameplay. Critic calls are audited in `llm_calls` tagged `call_kind=critic` (with thinking captured under `LOG_LLM_THINKING_ALL`, like decision calls).
- **YAML asset schema validation (fail-fast at boot + tests)** — every shipped char-creation asset (`classes`, `backgrounds`, `races`, `alignments`, `day-jobs`, `item-sets`) is now validated against a hand-rolled schema (`src/assets/asset-schemas.ts`) when loaded, so a malformed file (e.g. a background missing a stat key) crashes boot with a precise file+entry+field message instead of silently flowing a `NaN` into `computeStats` and persisting a `null` ability score. New tests (`tests/assets/`) validate the real files: schema (T1), four-integer modifier completeness (T2), a `computeStats` finite round-trip over every class×background×race (T3), cross-file integrity (kit coverage, day-job `workplace_location` ⊆ seeded locations, unique alignment cells — T4), a release-notes sweep with tag=filename (T5), and golden per-file entry counts. Seeded locations are now exported from `migrate.ts` as the single source of truth.
- **Weekly recap thread (Monday rollover)** — each week gets one pinned **header** message in the play channel with a **thread** hanging off it; all public action outcomes now post into that week's thread instead of the channel feed (the private/ephemeral outcome the acting player sees is unchanged). On Mondays — with the nightly tick (03:30 UTC) so the week boundary aligns with the daily action refresh — the previous week's placeholder header is rewritten in place into a short LLM-generated **chronicle** (a 2–4 sentence world digest + notable highlights, bland actions ignored) via a new recap prompt, and a fresh week (new pinned header + thread) begins. Headers are kept pinned as a running archive. The recap is best-effort: on any LLM failure it finalizes with a deterministic count summary, and if a thread is ever unreachable, outcomes fall back to posting in the channel so none are lost. A current-week thread is also created on boot so outcomes always have a home — recreated only when the stored thread was *genuinely* deleted (Discord `Unknown Channel`/10003); a transient fetch failure on boot (rate limit, 5xx, network blip) keeps the current week rather than spuriously rolling it and orphaning the in-progress chronicle. **Requires the bot to have _Create Public Threads_ + _Send Messages in Threads_** (in addition to _Manage Messages_ for pinning).
...
### Fixed
- **Recurring `DiscordAPIError[10062]` (Unknown interaction) across buttons** — two source-level fixes. (1) A **global in-flight guard** now wraps every component/modal interaction at the dispatcher (`src/index.ts`), keyed per `(user, source-message)` for buttons and `(user, customId)` for modal submits: a duplicate click while the first is still processing is silently `deferUpdate`-acked and dropped, instead of racing the already-edited/deleted source message into a 10062. Previously only the `/join` wizard deduplicated clicks; now all buttons do. (2) A shared **`isDeadInteraction`** helper (codes `10062`/`40060`) makes dead interactions a uniform no-op: `notifyAdmin` no longer DMs/red-logs them (quiet `VERBOSE` trace only) and `safeErrorReply` no longer warns when the interaction it's trying to surface an error on is itself already gone — killing the "could not surface error to user" double-log. The per-call `code !== 10062` check in the join branch is now redundant and removed.
- **Daily-work pay & presentation** — clicking a preset day-job task now actually pays its per-action `income` (the "bonus for clicking"), paid into the **resolved outcome** so it shows as a `💰` delta in the footer (after the failure-strip, so it's earned win or lose; not paid if you bail) instead of being silently dropped or fired as a separate pre-work message. The daily-commute `−1 stamina` is shown merged with the "thinking" page (no longer a stalled-looking or duplicated note). Preset work is labelled **`Work:`** (with the profession's emoji) rather than `🧭 Quest:`, via an `ActionKind` carried through the whole action incl. multi-beat resolution. (`base_income` remains the separate automatic nightly-tick wage.)
...
### Changed
- **Critic backfills the flagged decision's own audit row** — when the critic flags a beat, the *critiqued* decision/narration call's `raw_prompt` + `reasoning` are written onto its own `llm_calls` row (held transiently on the decision, applied via `promoteDeepCapture`, COALESCE so it never erases an existing capture). So the rejected output is fully mineable on its own row, not just inferable from the critic's prompt. Added `critic_flags` / `critic_rate` query shortcuts to `scripts/query.mjs`.
- **Critic verdict is now a queryable column + always audited when it flags** — `llm_calls.critic_severity` (`ok`/`minor`/`major`, NULL on decision calls) surfaces the coherence critic's verdict without parsing `response_json`, so flags are minable with a plain `WHERE`. And when the critic flags an issue (`!ok`), its thinking + raw prompt are always kept regardless of `LOG_LLM_THINKING_ALL` (the critic prompt embeds the critiqued decision, so the rejected output and the reason are captured together). Note: this is distinct from `validation_warnings`, which is the *deterministic* per-decision validator, not the critic.
- **Auto-capture "spiral" calls** — any LLM call whose reasoning exceeds `REASONING_SPIRAL_CHARS` (default ~6000, the observed p90) always persists its thinking + raw prompt, even with `LOG_LLM_THINKING_ALL` off — a long chain is itself the signal worth keeping. Tunable via env; applies to decision and critic calls.
- **Fuller LLM audit for data mining** — `LOG_LLM_THINKING_ALL=true` now also captures the full `raw_prompt` on every well-formed call (previously thinking only; the prompt was diagnostic-only), so the exact v9 markdown sent to the model is queryable. And **every** `llm_calls` row an action produces — each decision beat, the narration, and the coherence-critic calls (decision-beat + resolution) — is now linked to that action via `action_id` (previously only the single final call was), so the whole call chain for an action is mineable. Idempotent backfill at resolution; no schema change.
- **Dropped the legacy `actions.llm_request` / `llm_response` columns** — superseded by the `llm_calls` table (per-call `response_json`, `raw_prompt`, `reasoning`, tokens, latency, linked via `action_id`) and written NULL on every action since v4. Removed via an idempotent, guarded migration; per-call LLM audit is unaffected.
...
## [0.2.3] — 2026-06-19
### Added
- **Player-facing release notes** — a YAML file per release tag (`assets/release-notes/<tag>.yml`, e.g. `v0.2.3.yml`) holds non-technical "what's new" highlights. On boot, if the running tag (`v<VERSION>`) differs from the stored `last_release_announced` meta and a notes file exists, the bot posts the notes to the announcement channel with a **Request / Feedback** button (routes to `submitFeedback`), then stamps the meta so it fires once per tag. No file for the current tag → nothing posts (and the meta is left untouched, so adding one later still fires). Gated on `TICK_CHANNEL_ID`.
- **Saturday wilderness threat** — at **12:00 UTC** on Saturdays the afternoon beat names one unsafe location, spawns a themed hostile NPC there, and nudges players to go engage. The targeted location rotates week-by-week through the five wilderness spots (Dark Pines · Broken Keep · River Crossing · East Road · Forest Edge). New `WorldEngine.spawnNpc()` introduces NPCs from engine-driven events (no `created_by_action_id`). Idempotent per UTC day via `last_threat_date`.
- **Wealth + might leaderboards** — on **Wednesday and Sunday** at 12:00 UTC the afternoon beat posts two boards: richest characters by coin and mightiest by highest ability score. New `WorldEngine.getLeaderboards(limit)` returns both. Idempotent per UTC day via `last_leaderboard_date`.
- **Public collapse notices at 0 HP / 0 stamina** — when an action, rest, or the nightly tick drops a character to 0 HP or 0 stamina, a third-person notice is broadcast to the announcement channel so the whole table sees who fell (not a private DM). Fires only on the *transition* to 0 (no spam while bottomed out). Posting is decoupled via a registered broadcaster (`setCollapseBroadcaster`); the tick returns `collapsedNames: string[]` for a batched overnight announcement. There is no death mechanic — 0 remains a floor.
- **`hasRestedToday` on character data** — derived from the new `last_rested_day` column vs the current `day_number`; drives the Rest button's visibility.
...
- **Daily work blocked at unsafe locations** — quick day-job actions now refuse to start from an unsafe (or unknown/procedural) location with an in-character nudge to reach safer ground first. Freeform `/action <description>` is unaffected — only the day-job quick buttons are gated.

### Changed
- **Daily action allotment raised 2 → 3** — characters now start each day with 3 rolls (at creation and on the nightly tick reset).
- **Saturday bonus roll for everyone** — the Saturday (UTC) tick grants **+1 roll** on top of the daily allowance (4 total that day). Tied to the real-world weekday of the tick, not the game day number. The Saturday threat announcement calls it out with a one-liner.
- **"Sleep" nav button renamed to "Rest" (🏕️)** — and it now hides once you've actually rested for the day (previously it lingered while out of rolls, letting you re-trigger it). The internal command id stays `sleep`, so routing is unchanged. Visibility: shown only when out of rolls, idle, and not yet rested today.
- **Custom day-job action dismisses the menu immediately** — pressing **Custom…** now deletes the stale day-job menu message as soon as the modal opens, rather than waiting for the modal submit.
- **Afternoon beat scheduler (12:00 UTC)** — a new daily scheduler dispatches by UTC weekday (Sat → threat, Wed/Sun → leaderboards); gated on `TICK_CHANNEL_ID` alongside the existing tick/morning/goodnight schedulers.
...
### Fixed
- **Stale stats on auto-finished actions** — the custom-action modal and day-job quick-action paths rendered their outcome message from the pre-action character snapshot, so an auto-resolved (travel/rest) action showed the *old* roll count and stamina, and the nav bar offered **Action** when the player was actually out of rolls. Both paths now re-read the character after `startAction`, so rolls (`🎲 x/2`), stamina, and the Action→Sleep button swap reflect the spent roll and applied mutations. The `/action <description>` slash path was already correct.
- **Missing Feedback / Bug Report buttons on auto-finished actions** — those same two paths attached only the nav buttons; the 💬 Feedback and 🐛 Bug Report buttons now appear on auto-finished outcomes too (matching the slash-command path).
- **Mid-action resolve no longer dead-ends** — `done` was deprecated from the decision prompt (v7) in favour of an empty `decision` array meaning "resolve now", but the action state machine's `step()` still gated resolution on the dead `done` flag. When the LLM resolved a committed action mid-beat it degraded to a lone "Step back" option; `step()` now infers resolution from no real options, mirroring `start()`. Also dropped a stale `done: true` instruction from the runtime narration prompt and reworded a validation warning that false-positived on every legitimate resolve.
