...
- **`scripts/send-dm.ts` can carry embeds (`--embed <file>`)** — a JSON embed, or an array of them, sent with the body or on their own, because a plain body has a single 2000-character budget for everything a message carries: one that must hold per-item cards otherwise loses either the items or the links. The embeds are validated against Discord's limits (title 256, description 4096, field name 256, value 1024, 25 fields, 10 embeds, 6000 across the message), so an over-long part is refused by name and count, and refused as a reason rather than a stack trace, the way an over-long body already was.
...
- **Meta-oil's digest becomes cards** — the digest is one message of embeds: a neutral index card carrying the signals, the window, the spend, what is still open and the reaction legend, then one goldenrod card per proposal carrying its six labelled lines, so each proposal reads, quotes and answers on its own instead of competing with the others for one body's 2000-character budget. Every issue, PR and file named is an absolute link, and the link sits in a card's body rather than its title: Discord renders a masked link in a description and prints it literally in a title, verified against a live DM. `.pi/factory/memory/` is never linked (gitignored, so it would 404), and the full drafted edit is written to `meta/proposals/<date>.md` before the card goes out, since an approval is applied days later in a fresh context. Scrumo stays on plain content: three actions need no cards.
- **One digest can approve some proposals and reject others (`scripts/factory-inbox.ts`)** — `❌` now means *the rest* instead of *all*, so `1️⃣` `2️⃣` `❌` is approve 1 and 2, reject 3, and `❌` tapped alone still rejects everything. The drain resolves the taps into **one verdict per proposal** (`## Verdict` in the brief and in `--json`) rather than leaving a combination of reactions to the agent's reading, and only reactions fresh in this drain count so a digest drained twice cannot be applied twice. `--record <id> --seed <n>` now stores `n`, which is what lets a verdict name the proposals the owner never tapped, and recording a new digest clears the previous message's reaction baseline, which previously discounted a fresh approval on a new digest to zero. The intake module also no longer drains on import, so it can be tested without reaching Discord.
