...
- **`dead-end` stops counting children the lead commits for** — a lead-driven `delegate-executor` or `delegate-fixer` child is contracted never to commit, so the predicate flagged every one of them (33 of the window's 39 while the job ledger showed 25 of 26 stages first-try ok); it now asks whether the session owed a commit, and a ledger stage still dead-ends if it edits and refuses to. `git -c … commit` no longer slips past the shipped check either.
