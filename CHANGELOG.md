...
- **Factory loop cadences move to clock times, and scrumo splits in two**: triage 07:30/19:30, executor 06:00, sweeper 21:00 every second day, and scrumo 19:00 twice a week as `factory-scrumo` (Tuesday) + `factory-scrumo-thu` (Thursday), all local time. A Tuesday/Thursday pair is not expressible as one fixed interval, so it needs two schedules. Every deepseek call now also lands outside the provider's 01:00-04:00 and 06:00-10:00 UTC peak windows, where the same tokens cost half as much, and the seven schedules are **enabled** on the box — runtime state under the gitignored `.pi/subagents/`, so a clone inherits neither the cadences nor the enabled flags — which is what lets the factory run itself through the headless launcher. Clock times are approximate: pi has no calendar trigger, so each is an anchored interval and the systemd tick quantises the start, which is why that tick drops to 5 min and `scripts/factory-run-due.sh` now skips a tick with nothing due rather than spawning a pi to be told so.
