...
- **A bail now says whether the day's free step-back was used** — the once-per-day bail refund was invisible in the 🎲 footer, so the second same-day step-back read as a bug rather than as a rule. A refunded bail adds "first step-back today is free"; one that kept the roll adds "the free step-back is already used today". Only the bail path says either, so timeouts and no-op refunds keep their own in-voice messages.
