...
- **The retired DeepSeek model aliases go**: every pin moves off `deepseek-v4-flash` and `deepseek-v4-flash-vision-exp` onto `deepseek-flash`, the model those requests were already being served by and billed as (DeepSeek V4.1 Flash), and triage's `xhigh` becomes the `max` it was already being clamped up to, since the direct provider carries no `xhigh` tier.
