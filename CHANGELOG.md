...
- **Coherence critic can now be gated to risky beats, and an agent-player run reports what the LLM cost** *(RA-4)* — the critic fires on every decide and narrate beat, ~35% of prod LLM calls for a step players never see. `CRITIC_GATE_MODE=anomaly` restricts it to beats an anomaly predicate flags (`src/engine/action/critic-gate.ts`); the default `always` is today's behaviour unchanged, pending the measured keep/gate/drop call. `ENABLE_COHERENCE_CRITIC` remains the global off-switch and is now honoured by `npm run agent:play` too. A run prints an LLM-cost summary (calls/tokens per `call_kind`, critic verdict spread, and an exact count of verdicts that could change anything), backed by a new nullable `llm_calls.beat` column recording which beat a critic call reviewed. **Also fixes a latent gap found on the way:** the agent-player harness never wired a coherence critic at all, so every run to date silently skipped it regardless of `ENABLE_COHERENCE_CRITIC`.
