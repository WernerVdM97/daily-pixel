...
## [Unreleased]
### Added
- **`/map` — your map of the world** — an ephemeral, region-grouped hub-and-spoke tree drawn with box-drawing connectors (`├─ │ └─`) so levels read on mobile, with a "roads not yet walked" section for frontier exits and a per-line `emoji · safe/wild · effort` glyph row. Progress is a count ("N charted · M roads into the unknown"), never a fraction; `/map <region>` drills in; over-long maps collapse a region tail into `+K more` and never silently truncate. A 🗺️ **Map** button cross-links from the other info pages (Hi/Journal/Stats/Backpack/Look).
