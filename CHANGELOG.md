...
- **The roadmap update deck (`docs/assets/roadmap-update-2026-09-22.html`)** — one self-contained, offline-openable status deck for the owner: focus milestones B and A, Release A's blocker bands, the panel's five themes with verbatim quotes, the factory cost panel, and the nine owner decisions with one-line questions. Built from live repo and board data as of 2026-09-22 and revised the same evening after live html-explainer and work-canvas review passes (chart geometry, decision surface) plus an agent-smoke breadth re-derivation, which held the invariants and filed two harness defects (#172, #173).
- **The Dark Factory spends on its own OpenRouter key (`FACTORY_OPENROUTER_API_KEY`)** — read from the process environment or the repo `.env` (never sourced) and passed as `--api-key` at both factory `pi` spawn sites (the launcher's schedule fire, `scripts/factory-run-due.sh`, and the job ledger's stage wrapper in `scripts/factory-jobs.ts`), so factory spend is separable from the shared `~/.pi/agent/auth.json` credential on the dashboard. `--api-key` is omitted when the key is empty, so the shared credential stays the documented fallback the launcher warns about, and a `GET /api/v1/key` preflight reads the key's own `limit_remaining` and skips the tick on a definite zero (failing open when the budget cannot be read). To set it: put the key in the repo `.env` and cap it on the OpenRouter dashboard.
- **A spec for OpenRouter key separation and cache observability (`docs/engine/openrouter-keys-and-cache-observability.md`)** — the two loose ends the route change left: who spends on which key (the factory's 11 agents still ride the shared one), the `--api-key`-beats-`auth.json` precedence trap that makes an exported env var useless, and the five `llm_calls` columns that make the host pin provable. Decided direction; the key split is built in #160.

- **The agent-player plays a persona panel (`AGENT_PERSONA`, `npm run agent:panel`)** — the harness stops being one live run repeated: the brain gains working memory (a day-start recap of yesterday's outcomes and disposition, today's day log with rejections included, a persistent intent note and an `arcNote`) and a free `recon` move (`/look`, `/map`, `/stats`, `/backpack`, `/journal`, `/help`, capped per screen and per day, no roll, no day advance); both prompt families go set-based at `v2` with a first-player handbook whose roll figures a test pins against the engine constants, and ten persona fragments (`AGENT_PERSONA=explorer|socialite|soldier|homesteader|grinder|collector|storyteller|tourist|casual|lapsed-returner`; unset = no fragment = the baseline arm) stamp `agent-v2/<name>` on the protocol header. Each turn may carry a `friction` line tagged `once`/`periodic`/`ritual`, and each day a note with engagement and fulfilment ratings plus the updated `arcNote`, folded into the turn the last roll is spent rather than the sleep pick so a day ending `no-rolls` is still rated (an unrated day logs a warning finding); one persona-voiced review answers return-tomorrow, what-would-I-miss and what-would-make-me-quit against the five-criterion rubric, printed to stderr and written to `<AGENT_OUT>.reviews.json`; and `npm run agent:panel -- <dir>` aggregates a directory of those files into `panel.md`/`panel.json` (score and rubric matrices with coverage, the engagement/fulfilment series, friction ranked by projected exposure, per-persona verb histograms, an `arcNote`/`quitTrigger` distinctiveness check). Multi-day runs are real because the clock is pinned to `AGENT_START_DATE` and steps one game day per nightly tick (`pinAdvancingClock`, `src/agent/clock.ts`), so the Saturday bonus roll, the weekend greeting and the five-day absence nudge read the game's calendar; the same value is the header's `recordedAt`, so a recording and its replay agree, and replay steps one day per tick marker, which discharges the same-weekday-class caveat for multi-day streams. `AGENT_SKIP_DAYS=<n>` plays day 1, advances the world `n` days with no play at all (the lapsed-returner interruption), then plays the remaining days.

- **The agent-player can be told to take a free action (`AGENT_FORCE_FREE_ACTIONS=1`)** — each day's first menu offers the free-text slot only (the day-job buttons leave the brain's screen and its move list together, so the two numberings still agree) until a free-text (non-work) action completes, because the brain otherwise plays day-job work almost exclusively and `stripWorkInspiration` removes every inspiration grant on `kind: 'work'`, leaving RA-2's dial unreadable from a run. A bailed pick resolves the action without rolling, so it neither counts as the day's free action nor discharges it. Off by default (the menu is unchanged when unset), and the day summaries now print each day's free-action count.

- **`factory-control`, the skill for stopping and starting the Dark Factory** — the four levers (`.pi/factory/PAUSED`, `FACTORY_ENABLED` in `.env`/the unit/a drop-in, a schedule's own `paused` flag, the systemd timer), what each one does and does not stop, how to fire one loop or one tick by hand, and the tick test that proves a stop took. A stop is a claim about the tick rather than about a file, so the skill ends on the reading that distinguishes stopped, refused for memory, and nothing due.
- **`factory-control` gains the resume runbook** — a resume is a burst by construction (`run-due` only attaches a child per due schedule, so one tick starts every overdue loop), the pile is not eight jobs (three `factory-scrumo` schedules, two `meta-oil`), the two ways to avoid it (drain by hand, or reset `nextRunAt` to the next anchor and let the pile go), and the fact that nothing on this box caps concurrent async runs, while `maxActiveAsyncRunsPerSession` fails rather than queues if it is ever set.
- **One board item is now a tracked job with a 100-minute budget (`scripts/factory-jobs.ts`)**: a job is a record in `.pi/factory/jobs/<item>.json` whose stages run one per process — `build` / `review` / `fix` as agents with their own timeouts (50 / 20 / 30 min), then `deliver` / `reconcile` / `done` as plain code. That moves the two steps that keep getting lost out of prompt prose: `deliver` pushes the branch and opens the PR to `dev` with `Closes #n`, and `reconcile` closes the card and the issue once the owner merges. A killed drainer costs one attempt and its elapsed time, never the job; a second failure at one stage, or a spent budget, sets `Blocked` and pages instead of retrying for ever; and an item left `In Progress` with committed work is **adopted** at `review` rather than rebuilt. `factory-executor` becomes a starter agent that runs one command, and the sweeper's two board-hygiene rules stand down for items that have a job record. Spec: `docs/engine/dark-factory-job-ledger.md`.
- **`scripts/send-dm.ts` can carry embeds (`--embed <file>`)** — a JSON embed or an array of them, sent with a body or on their own, because one plain body has a single 2000-character budget and a message carrying per-item cards would otherwise lose either the items or the links. Every part is validated against Discord's limits (title 256, description 4096, field name 256, value 1024, 25 fields, 10 embeds, 6000 across the message), so an over-long one is refused by name, count and source file rather than as an API form error that names neither.
- **`scripts/factory-friction.ts --errors [n]` lists the failed calls behind a signal** — tool, session and the error's first line, newest first, in the text brief and under `--json`, because dissecting the top signal by hand cost more turns than the analysis it fed.
- **The friction report gains outcome metrics and per-loop attribution** — every session is labelled with the factory loop that ran it (subagent name, else scheduler brief, else `interactive`) so each signal offender names a loop rather than a bare session id and tier changes are priced on the loop that burns the tokens; a `Job ledger` section summarises executor delivery from `.pi/factory/jobs/` (first-try stage pass rate, retries, budget burn); and `--board` lists Blocked items whose latest comment is the owner's — the #54/#55 case, an answer sitting unprocessed while triage passes over it. `.pi/agents/meta-oil.md` carries the matching brief: measure outcomes as well as transcripts, diagnose `dead-end` and `file-rework` before `tool-error`, and re-check a proposal's file list against the live branch before applying (the `.claude` → `.pi` lesson). Tests: `tests/scripts/factory-friction.test.ts`.

### Changed

- **The factory triages dependabot instead of ignoring it** — triage opens each pass with a dependency check over the open automation PRs: one assessment comment per PR, a `@dependabot rebase` request when a stale base is what is red, and a migration card (one issue per dependency, `dependencies` label, scoped through the gate) for a failing or breaking major; the sweeper's dependency audit checks coverage (CI, age, migration card present) rather than repeating triage. Green grouped minor/patch bumps stay report-only and no agent merges, pushes to, or approves any of these PRs — merging a dependency remains an owner step. Loop definitions in `.pi/agents/factory-triage.md` / `factory-sweeper.md`, rationale in `docs/engine/dark-factory.md`.
- **The game reaches its model through OpenRouter now, and the direct DeepSeek API route is gone**: `deepseek-transport.ts` becomes `chat-transport.ts`, `DeepseekLlmGateway` becomes `ProdLlmGateway`, `callDeepseek` becomes `callChatCompletion`, and `api.deepseek.com` is no longer called from anywhere. Every request carries `provider: {order: ["deepseek"], allow_fallbacks: false}`, so the DeepSeek host always serves it: one game turn re-reads the same system prompt and context prefix on every call, and a host switch mid-session reprices the whole prefix, which costs far more than the spread between hosts. `allow_fallbacks: false` means a DeepSeek outage is a visible error the caller already handles, not a silent re-route onto a cold cache. The pin is also the only reason the fallback matters at all: DeepSeek is the one endpoint of the model's 17 that OpenRouter flags `supports_implicit_caching`, so any other host would not merely be cold at the switch, it would never cache the prefix.
- **The default model moves a version with it, `deepseek-v4-flash` to `deepseek/deepseek-v4.1-flash`**, because a pin needs a host to pin to and no DeepSeek endpoint serves the old slug. It is a different model, not a re-route: prompt tokens go from $0.084 to $0.15 per M and completion from $0.168 to $0.60, against cache reads that fall from $0.0168 to $0.003, which is what pays for it given how much of a game turn is a re-read prefix.
- **Two wire differences are silent traps, and the transport handles both in the one place**: chain-of-thought is requested with `reasoning: {enabled: true}`, because DeepSeek's native `thinking: {type: 'enabled'}` is accepted and ignored on this hop and the call comes back with no reasoning and no error; and it arrives on `message.reasoning`, because OpenRouter normalises every vendor's chain-of-thought into that field and DeepSeek's `message.reasoning_content` is gone. Both are pinned by tests (`tests/llm/pipeline/prod-gateway.test.ts`), including one that fails if the request body ever loses the provider pin or starts posting to the DeepSeek URL.
- **Omitting `reasoning` is not "reasoning off", so the field is now sent on every call**: the pinned model reasons by default (`default_enabled: true`, and its efforts are max/high/low with no `off`), which means the three stages that never opt in — classify, resolve-mutate, resolve-narrate — were silently buying a chain-of-thought, billed as completion tokens and emitted before the content. Probed on the live API: a silent body returned 28 reasoning tokens, the same body with `{enabled: false}` returned none. An unflagged call now sends an explicit off, and a test fails if it ever goes back to silence.
- **The player-agent runs on its own OpenRouter key (`AGENT_OPENROUTER_API_KEY`), and refuses to start without it**: `agent:play` explicitly does **not** fall back to the bot's `OPENROUTER_API_KEY`, so a mistyped variable is an error rather than a playtest quietly spending the bot's quota. The bot's own key replaces `DEEPSEEK_API_KEY`, the harness model override is `AGENT_MODEL` rather than `DEEPSEEK_MODEL`, and `.env.example`, the README, the deploy doc, `provision-lxc.sh` and the `agent-smoke` skill all name the new variables.
- **The user-facing LLM error strings follow the provider, and `ErrorMapper` matches them case-insensitively**: the `OpenRouter API error` / `OpenRouter returned empty response` / `Failed to parse OpenRouter response` prefixes are what `ErrorMapper` matches to turn a stage failure into "The warden's vision is clouded", so leaving them reading `DeepSeek` would have quietly degraded every LLM failure to the generic fallback message. The match was also case-sensitive while only the v11 gateway capitalises `Failed to parse` — the pipeline and both agent gateways throw it lower-case, so their parse failures went to the generic fallback anyway.

- **Every DeepSeek agent pin moves onto OpenRouter, held to the DeepSeek host**: the seven factory and delegation agents (`delegate-{coordinator,executor,reviewer,fixer}`, `factory-scrumo`, `factory-triage`, `meta-oil`) and the job ledger's stage wrapper (`WRAPPER_MODEL`) pin `openrouter/deepseek/deepseek-v4.1-flash`, and the `only: ["deepseek"]` / `allow_fallbacks: false` override in `~/.pi/agent/models.json` fixes the serving host, because a host switch means a cold prompt prefix on every request and the cache-read rate is what makes a long run affordable. This reverses the direct-DeepSeek-only rule and leaves the direct provider pinned by nothing; `meta-oil` and `factory-triage` move from `max` to `xhigh`, OpenRouter's ceiling, rather than being silently clamped. No `fallbackModels` is wired on purpose: the factory runs on one balance, and a drained one should stop it rather than be routed around.

- **`tool-error` now excludes deliberate probe exits (`scripts/factory-friction.ts`)** — a failed bash call used as a boolean check (grep for absence, diff-as-check, `! cmd`, an existence test) is tallied separately as a probe exit rather than as friction, because deliberate behaviour is not confusion (24% of the 2026-09-12 survey's tool-error was probes). The count drops for that reason, not because the factory improved: compare windows on the genuine share, and probe classification is a heuristic, so `--errors` marks probes for a diagnosis child to re-check.
- **The bulletin is windowed to the next two milestones (`scripts/factory-bulletin.ts`)**: the board is 79 items and most of it is work for a milestone nobody is building, so the queue tables now show only the focus milestone and the one after it, ordered by milestone and then priority and age, with the rest counted in a `not listed:` line instead of listed (14 waiting on the owner and 26 ready to approve become 4 and 10). A `P1 - high` item anywhere stays on the page and sorts last, and an item with no milestone is never hidden, because a card with nowhere to live is a defect to see rather than a card to lose. No dated open milestone means no window and the whole board is shown, the same fallback the focus filter uses.
- **The ledger holds approved work it cannot run yet (`scripts/factory-jobs.ts`)**: approval answers "may this ever run", and the pick now also asks "now". It refuses an item carrying `needs-human-decision` (an `Approved` status no longer outranks a label that means a human decides first), an item with an OPEN native `blockedBy` dependency (read from GitHub, so a design card can gate its implementation instead of the link living only in a loop's memory), and an item outside the focus milestone, which is the open milestone with the earliest due date, cached at `.pi/factory/focus.json` for triage to read (no dated open milestone means no filter, and priority order decides). Nothing is dropped silently: `start` returns a `held` list naming a reason per item, the executor relays it, and the pinned bulletin prints the focus and the held count, so an idle factory is explained rather than left looking broken. Spec: `docs/engine/dark-factory.md` § The gate.
- **Scrumo refreshes the pinned bulletin on every pass** — the sweeper ran it only every 48h, so the `[bulletin]` link in a digest could read up to two days stale (it once said "waiting on you: 0" while two PRs sat green). `factory-bulletin.ts --post` is now step 1 of scrumo's digest and its one sanctioned write; the sweeper's job 5 stays as backstop.
- **`.claude` is deprecated; the repo's skills live in `.pi/skills/`** — pi loads project skills from `.pi/skills/` and never read `.claude/skills/`, so all 14 skills were invisible to pi runs and reachable only through the hand-written AGENTS.md path table. They move wholesale (git history preserved), `CLAUDE.md` and the `.claude/` directory are deleted, and the stale pointers follow: the AGENTS.md table, `.gitignore` (`.pi/skills/` joins the tracked set; `.claude/worktrees` goes, because pi's managed worktrees live outside the repo), the `vitest.config.ts` `.claude/**` exclude, `factory-escalator`, and the factory memory README. `docs/archived/` and the requirements doc keep their mentions as history.
- **The retired DeepSeek model aliases go**: every pin moves off `deepseek-v4-flash` and `deepseek-v4-flash-vision-exp` onto DeepSeek V4.1 Flash, the model those requests were already being served by and billed as. The route and the thinking tiers are the OpenRouter entry above.
- **Factory loop cadences retuned again** — triage `12h` at 07:30/19:30 (was 06:00/18:00), executor `24h` at 06:00 (was 04:00), which moves both off DeepSeek's peak windows (01:00-04:00 and 06:00-10:00 UTC, Mon-Fri; off-peak is half price). Unchanged: sweeper `48h` at 21:00, scrumo 19:00 Tue + Thu + 14:00 Sun, meta-oil 20:00 Fri + Sat. Retuning means editing `schedule.json` directly, because no tool action updates a cadence.

### Fixed

- **A provisional location the cartographer failed to place is retried on a bounded sweep rather than stranded** — a transport or parse failure came back as an empty *success*, which settled the row with the placeholder text and cleared its `enrichment_pending` flag, and the one path that did leave a row pending for ever (an `enrich()` that threw) had nothing to re-attempt it. `enrich()` now reports failure as `undefined` and the row stays provisional, and the nightly tick sweeps pending rows back to the cartographer (3 per tick, oldest first) up to a 3-attempt cap held in the new `locations.enrichment_attempts` column; at the cap the row settles with the placeholder so an unmappable name cannot retry indefinitely.
- **The Dark Factory's `review` stage can launch again** — `delegate-reviewer` asked for `web_search`/`fetch_content`, which an ambient extension provides and a foreground child never loads, so pi-subagents' strict allowlist aborted every review 37ms in, before its first model call, and blocked job #97 after two attempts that never produced a finding. The reviewer now declares builtins only; `delegate-judge` keeps its web tools by loading the provider itself through `subagentOnlyExtensions`; a test pins both rules against the agent definitions.
- **A fight's DC is pinned when the fight opens, so the combat card and the narration agree for the whole fight (`in_combat`'s new `baseDc` prop)** — every CONTINUE round re-authored `baseDc`, and that one number feeds both `enemyBonus = clamp(baseDc - 10, 0, ENEMY_BONUS_MAX)` and the card's `dangerTier`, so a later round of a fight could show one tier, fight at another, and move the foe's to-hit bonus mid-fight for no in-world reason. The opening round still authors it; the engine now writes it to the `in_combat` edge and every continue round reads it back (an edge persisted before the prop existed falls back to that round's own authored value), and the continue decide message is handed the number as `- Fight DC: <n>` rather than being invited to move it. Unblocks #96: the tier band now describes a foe, not a round.
- **A failed brain call costs a turn, not the run** — found by the first live arc panels: the homesteader arc crashed on day 3 of 5 when the model returned JSON with no `choice` field, the gateway's fail-loud `resolveMove` threw, and `playOneAction`'s catch turned it into a fatal `crashed` that stopped the run, voiding four days of intended measurement. `ask()` now re-asks **once** on a failed call — the re-prompt the seam's own contract always claimed, and the cheapest correct answer to a provider timeout — records both attempts' `llm_calls` rows, and downgrades a recovered slip to a warning finding; if both attempts fail the turn is abandoned as a non-fatal dead-end instead of a crash, so only a persistent fault trips the stuck counter and stalls the day. A run whose day ended `crashed` or `stalled` also exits 1 with `run ended early` on stderr, because a five-day arc that died on day 3 previously exited 0 and the exit code is the only thing QA automation reads.
- **`/help` stops teaching the wrong roll count and wizard length**: the command list said "2 rolls per day" (the allowance has been 3 since `0.2.3`, and 4 on Saturdays) and "6-step wizard" (the wizard's own footer counts seven), so the one screen a new player trusts was the one screen still wrong. The same retired figure sat in `docs/vision/pitch-and-pillars.md`, which also handed the Saturday bonus to the whole weekend; both copies now read three rolls a day, four on Saturdays. The screens oracle and `help.test.ts` re-pinned.
- **`factory-bulletin.ts --json` emits parseable JSON again**: the ledger's log lines were reaching stdout, which is that script's payload, so `--json` came back unreadable and `--post`'s one-line result sat under a log line. The ledger's log now goes to stderr.
- **The friction report stops counting a fork's replay of its parent** — a fork copies its parent's transcript verbatim under the same entry ids, so every failure, command and token in the parent was counted again per fork (62 sessions and 126 failed calls read as 55 and 87 on the same window). Entries whose id appears in an ancestor's transcript are skipped before any counter runs, and offender rows carry per-session counts, so the session named beside `5x edit` is the session that holds the five.
- **A stopped Dark Factory can no longer be restarted by opening a session here (`factory-control`)**: `.pi/factory/PAUSED` is read by `scripts/factory-run-due.sh`, so it stops that script's schedules, drain and housekeeping and nothing else — pi's own schedule runner fires an overdue project schedule from inside any session in this repo, so a 2026-09-14 stop still let a triage pass write the board as the account the pause was protecting, six seconds after the session opened. A stop is now two levers, the pause file for the tick and `schedule.pause` per loop for the session path, and the skill's Starting and Verifying steps read both (`subagent({action:"schedule.list"})` is the check the journal cannot give you).

### Internal

- **The comment sweep reaches `src/engine/`** — `PipelineActionStateMachine.ts` goes from 468 comment lines to 217 and loses every block over two lines, keeping the load-bearing set (why a `first-real` beat needs its lethal option listed first, why sparing writes a 0-HP edge rather than a guaranteed-win 1-HP survivor, the `set_relation` type the combat edge needs or `validateMutations` drops it, and `enemyConditionBand`'s NaN tier). `WorldEngineImpl.ts` follows in the same batch: 407 comment lines to 176, 30 blocks of four or more lines to none, and five comments retired for asserting behaviour the code no longer has. Comment-only, so the comments-removed transpile output is byte-identical to `dev`'s. Batch 2 does `src/agent/`: 2,148 comment lines to 1,109 across 24 files, and it retires a family of comments naming code that no longer exists (a deleted legacy machine, a deleted state type, a guard nothing implements). Batches 1 and 2 of 7, tracked in #177. Batch 3 takes the Discord surface — `src/discord/`, `src/render/` and `src/view/` — from 1,473 comment lines to 779 across 35 files, in five units because the plan's own file list was short: the 13 files under `src/discord/commands/` had been assigned to no unit. It retires another family of comments that asserted what the code does not do, among them a module called the only one that knows about `discord.js` (six others import it), a palette said to mirror a colour map held in a file that holds none, a "warm" palette variant whose `player` channel goes magenta, an enemy bar width described as always fixed after a re-entry branch had made it adaptive, an `hp/maxHp` pair described as inert while they still print, and a hint comment listing three gates where the code has four. Batches 1 to 3 of 7, tracked in #177.
- **Comments stop being a second changelog**: `AGENTS.md`'s one-line rule ("explain why, not what") became hard rules, a comment earns its place only by saying what the code cannot, in 1 or 2 lines at most, with `#NNN` and board references, decision narration, a rationale repeated across several layers, echo comments and cross-file line citations all banned, and exported-helper usage docs plus test-fixture scenario notes named as the carve-outs. Enforced where the factory loop actually reads it: a new `code-comments` skill, a comment-discipline finding category in `delegate-reviewer` (which inherits no project context, so the bar is repeated inline), and a short line in the executor and fixer contracts. Triggered by PR #175, roughly 10 lines of behaviour sitting under 34 lines of comments that stated one fact at eight layers.
- **`docs/CONVENTIONS.md`'s marker table teaches the list-item form and closes the set**: the markers were listed bare, so the vault drifted into a bare `[!]` / `- [!]` split (bare is prose to Obsidian, not a list item). § List markers now shows `- [!]` and carries an NB: a new marker letter is added to the table before the doc that uses it lands. The three live docs still carrying bare markers are swept to match; `docs/archived/` keeps its own, as history.
...
- **The tick keeps the checkout current and prunes merged branches (`factory-jobs.ts housekeeping`)** — every tick, after the drain: `git fetch origin --prune`; fast-forward the local `dev` ref when that is provably safe (through the working tree only if the checkout is on `dev` **and** clean, otherwise the ref alone); and delete local branches whose work is provably in `dev`: wholly contained in `origin/dev`, or a merged PR whose head was **exactly that branch's tip** — a name matching a merged PR is not proof, since `feat/<item>-<slug>` is deterministic and a redone item recreates a name an older PR already owns. A branch that moved after its merge is kept with both SHAs logged. The predicate cannot be ancestry alone, because this repo squash-merges: a merged branch is not an ancestor of `dev`, so `git branch -d` refuses it while a naive `-D` would delete real work. `-d` is tried first either way, and `-D` only follows a logged tip-equality proof. `dev`, `main`, the current branch, anything checked out in a worktree, and any branch a live job record names are never touched, and nothing remote is deleted. Jobs also fork from `origin/dev` rather than the local `dev` ref, so a stale ref can no longer become a stale base. The sweeper runs the same command and reports what it left behind.
- **One switch stops the whole Dark Factory** — `FACTORY_ENABLED=0` in the launcher's environment or in the repo `.env`, or the presence of `.pi/factory/PAUSED` (first line = the reason in the journal) stops every scheduled tick: no loop fires, no job stage advances, and the branch pruner does not run. Two levers, because a fire has two doors: the tick's schedules, and the schedule runner inside any `pi` session opened in this repo, which the file cannot reach and a per-loop `schedule.pause` closes. It gates autonomous action rather than the owner's: `FACTORY_FIRE=<id>` still fires the one schedule named, and `factory-jobs.ts start|drain|retry` still work when run by hand. **Absence means off**: nothing runs until some source says `FACTORY_ENABLED=1`, an explicit off beats an explicit on, and the pause file stops the factory even when it is enabled (it is the reason line, not the switch). This box opts in through its own gitignored `.env`. An in-flight stage finishes, because a tick already inside a 50-minute build cannot be interrupted safely.
- **Scrumo's digest is one linked message, and it goes out on Sunday too** — the DM is Discord markdown with a state line, `•` bullets and **an absolute link on every issue, PR, the board and the bulletin**, budgeted under 1800 characters because Discord rejects anything past 2000 and the whole digest would be lost (`send-dm.ts` now refuses an over-long body by name and count, instead of failing at the API with a form error that names neither). Each item appears in exactly one section, and the three decisions stay three, each an action with a link. A third weekly schedule (`factory-scrumo-sun`, 14:00 local) joins Tuesday and Thursday.
- **Meta-oil's digest becomes cards** — one message of embeds: a neutral index card (signals, window, spend, what is still open, the legend), then one goldenrod card per proposal carrying its six labelled lines, so no proposal competes with the others for one body's 2000-character budget; every link is absolute and sits in a card body, since Discord prints a masked link literally in a title, and `.pi/factory/memory/` is never linked because it is gitignored and would 404. The full drafted edit goes to `meta/proposals/<date>.md` in the same run, because an approval is applied days later in a fresh context.
- **One digest can approve some proposals and reject others (`scripts/factory-inbox.ts`)** — `❌` now means *the rest*, so `1️⃣` `2️⃣` `❌` is approve 1 and 2, reject 3, and the drain resolves the taps into **one verdict per proposal** (`## Verdict`, and in `--json`) rather than leaving a combination to the agent's reading; a `⏸` or `🔁` prints as *apply nothing*, and a proposal the owner never touched reads `no answer`. `--record <id> --seed <n>` stores `n` so that verdict can name the untapped proposals, and recording a **different** message clears the previous digest's baseline and its count with it, because every loop shares the one state file.
...
- **`TODO.md` is human notes only; agents file issues instead** — the file had spent a year absorbing agent scratch until its actionable items had to be hand-migrated to the board in bulk, and it still carried handover notes for shipped work. It is now fenced to human notes, and every agent-facing pointer to it follows: `AGENTS.md` says so in its file table and gains the prompt-verification lesson the file used to carry, the `agent-smoke` skill files friction and anomaly notes as `area:*`/`type:*` issues and takes the dropped concurrency finding as fleet guidance, the factory spec states that filing an issue is how an agent records work, and the harness spec, two roadmap docs and five source comments point at the issue that carries the item — the two no issue covers name the deferral itself instead. The remaining actionable content was migrated to issues #134 to #152, with the superseded items dropped and named in the file.
...
- **The factory grows scrumo + escalator and gets model tiering** — a fourth loop `factory-scrumo` (paused digest schedule, DMs the owner three recommended actions; needs `DISCORD_TOKEN` + `ADMIN_USER_ID` in the repo `.env`) and the `factory-escalator` heavy child (spawned by the executor for hard slices, `z-ai/glm-5.3` at `max` thinking). Models are pinned on all loop agents, direct DeepSeek only, never via OpenRouter: triage `deepseek-v4-flash-vision-exp` at `xhigh` (4.1-flash is not in the registry), scrumo `deepseek-v4-flash`, executor/sweeper `glm-5.3-flash`, escalator `glm-5.3`. The decisions move out of local `.pi/` into `docs/engine/dark-factory-requirements.md`; `.pi/agents/` and `.pi/factory/project.json` alone become tracked, while schedules, seeding payloads and memory contents stay local.
- **`meta-oil`, the factory's own improvement loop**: a sixth factory agent (pinned `deepseek/deepseek-flash` at `max`, the direct DeepSeek V4.1 Flash: the direct provider has no `xhigh` tier, so the thinking level is written as the one that actually runs) whose only subject is the machinery rather than the game. It measures with `scripts/factory-friction.ts`, diagnoses with read-only children, and DMs the owner at most five numbered proposals (prompts, verbosity, model tiers, epics, schedules, gate rules). Two paused weekly schedules run a survey on Friday and a confirm pass on Saturday. Applying a proposal needs an explicit token: one per run, in a worktree off `dev` on `chore/meta-oil-<n>`, PR to `dev`, never merged, and only the files that proposal listed. `src/` and `tests/` stay out of reach at any level of approval.
- **`scripts/factory-friction.ts` measures what the factory's own runs cost**: friction signals ranked by the tokens burned in the sessions where they fire (failed tool calls, the same command run 3+ times, the same file edited 4+ times, sessions that edited and never committed, owner corrections, aborted turns), plus cache economy read from the `pi-cache-optimizer` shards: per-route hit rates, full-price input tokens, and an effective $/M that weights cached tokens at the cache-read rate, so a cheap headline price with a poor hit rate is not mistaken for a cheap route.
- **The factory can now hear the owner (`scripts/factory-inbox.ts`)**: the loops could DM but never receive. Answers arrive three ways: inbox files under `.pi/factory/inbox/` (gitignored, works with no Discord at all), reactions on the last digest DM (1️⃣…5️⃣ approves that proposal, ✅/❌ all, 🔁 re-run, ⏸ hold), and DM reply text, which needs no privileged intent either: `MessageContent` gates *guild* messages, and Discord delivers DM content to a bot holding only `DirectMessages`, verified against this repo's own bot (it would only matter if the intake moved into a channel). Reactions are deliberately the primary channel: they are plain REST data on a message the bot sent, so a one-shot script reads them retroactively with no listener process and no privileged intent, whereas a button click is lost unless a gateway handler is connected at that instant. The digest is armed and pre-seeded with the vote reactions (`--record <id> --seed <n>`), so answering is one click, and the bot's own seeded reactions are excluded from the tally, so an unanswered digest reads as no answer. `scripts/send-dm.ts` now prints the `message-id` it sent (and `sendToAdmin` returns the ids) so a caller can watch for the answer, and `--record <id>` points the watcher at a digest.
...
- **The factory gets a bulletin, so "what is waiting on me?" has one answer (`scripts/factory-bulletin.ts`)**: a `Blocked` column cannot be read as a to-do list, because an item whose triage question was never written looks exactly like one that has a question, and that trains the owner to skim past real questions. The script sorts the human-gated work by what the owner can actually do with it (answer, re-read, fix the factory, approve, merge) and regenerates the single pinned issue titled `Dark Factory bulletin` in place, so the pin cannot go stale; `--post` writes it, `--json` emits the queue, and it needs no Discord token, so it works where the DM path is broken. The sweeper's job 5 now runs it instead of hand-rolling a digest.
- **The factory can fire its schedules headless (`scripts/factory-run-due.{sh,service,timer}`)**: a systemd timer runs the due schedules from outside any pi session, because firing one from inside a session stacks a second pi on a parent already holding ~700MB and twice ended in an `oom-kill` (`docs/engine/dark-factory.md` § Promoting to unattended). The launcher refuses to start below `FACTORY_MIN_AVAIL_MB` (default 1000) or while another tick holds its lock, and `FACTORY_FIRE=<schedule-id>` fires one named schedule by hand through the same two guards, for loops that are still paused and not yet due.

### Changed

- **Factory loop cadences move to clock times, and scrumo splits in two**: triage 06:00/18:00, executor 04:00, sweeper 21:00 every second day, and scrumo 19:00 twice a week as `factory-scrumo` (Tuesday) + `factory-scrumo-thu` (Thursday), all local time. A Tuesday/Thursday pair is not expressible as one fixed interval, so it needs two schedules. All seven schedules stay paused. Clock times are approximate: pi has no calendar trigger, so each is an anchored interval and the systemd tick quantises the start, which is why that tick drops to 5 min and `scripts/factory-run-due.sh` now skips a tick with nothing due rather than spawning a pi to be told so.
- **`Release A closeout` is `P1 - high`**: all six of its items set by the owner, with `.pi/agents/factory-triage.md` and the factory memory carrying the matching milestone-to-priority mapping. The old note that only two items sat above `P2` was wrong twice over: it read issue numbers as milestone numbers, and the count is now seven.
- **The sweeper stops flagging dependency PRs as gate violations**: the gate governs agent work on a board item, so a PR from an automation account with no linked issue is out of scope. It is counted in the digest instead (how many are open, how many have failing checks, the age of the oldest), because a major bump sitting for two months deserves one line rather than seven false violations that teach the owner to skim. The sweeper also flags board items with **no Status at all**, which no loop can see: triage only reads `Inbox`.
- **Dependabot stops eating its own queue**: `open-pull-requests-limit: 5` with one PR per dependency let five single bumps, all older than seven weeks, fill the entire npm queue, so nothing new could arrive from mid-July until they were cleared on 09-10/09-11. `.github/dependabot.yml` now groups minor and patch bumps per dependency type, keeps majors individual on purpose, and sets a `cooldown` (`default-days: 7`, `semver-major-days: 30`) so a version published this week is never proposed. The three stranded majors became board items: #104 (`js-yaml` 4.x to 5.x, a runtime dependency whose PR had been red for two months), #105 (`better-sqlite3` 11.x to 13.x, native ABI, verified against a prod snapshot rather than fixtures) and #106 (TypeScript 5.x to 7.x, two majors at once).
...
- **Agent-player becomes a protocol client (M6)** — `src/agent/harness.ts` speaks only `GameEvent`/`GameResponse` through `GameRouter` for the mid-day loop; zero controller imports remain in the action path. The envelope's `characterState` fact (`{ health, maxHealth, stamina, maxStamina, wealth, location }`) carries the character snapshot the brain reads each turn, populated by the router on every view-bearing response. `viewToText` reads envelope views; `charFromFacts()` builds `AgentCharView` from facts; `mapError()` routes every `GameErrorCode` to a `PlayResult` disposition; `onBeat` callbacks record commute beats. The existing M4 deterministic harness tests are ported to the protocol surface (20 tests, green). Bookends (`seedCharacter`, `endDay`) stay engine-direct until M7.
- **Bookend oracle coverage for the join/hi/sleep dispatch paths (M7.0)** — a new golden-transcript suite pins 16 dispatch-level transcripts over the three Discord bookends (wizard flow incl. confirm, /hi greeting + resume, unsafe-rest penalty, workplace exemption, guards, admin tick, character-gate reroute) so the M7.1–M7.3 seam migrations and the M9 rebuild diff against a byte-level baseline. Test-only; no production behaviour change.
- **Rest + nightly tick through the JSON seam (M7.1)** — the unsafe-rest −1 HP rule moves from the Discord `/sleep` handler into `WorldEngine.restAtOak` (value and conditions unchanged; the ⚠️ prose becomes router copy), the player goodnight crosses the seam as the new `rest.begin` event (controller `beginRest` guard order + `GameRouter` branch, no beats), and `/sleep` rewires to translate + paint, announcing the public collapse from the new `restUnsafe` fact. The agent harness's nightly rest now dispatches `rest.begin` too — its unsafe-rest −1 HP surfaces as a transcript finding for the first time — while the world `tick(true)` cron stays engine-direct. The admin-tick `/sleep` branch stays adapter-direct until M9 (flagged watch item). Contract suite extended for `rest.begin` in the same commit.
- **`/hi` through the JSON seam (M7.2)** — the greeting screen crosses the seam as the new `hi.open` event: the composition (character header, weekend hooks / day-job block, unfinished-action resume) moves into the controller layer, `GameRouter` gains a no-beat branch, and the `/hi` handler is translate + paint only. The charless reply unifies onto the other-events "You don't have a character. Type `/join` first." copy (the old "yet" copy was dead behind the character gate; the charless `nav:hi` edge follows the recorded M7.1 unification). Contract suite extended for `hi.open` in the same commit; M7.0 bookend-oracle transcripts 9–11 and the M1 `nav:hi` leaf stay byte-green with zero churn.
- **Character creation through the JSON seam (M7.3)** — the join wizard crosses the seam as `join.open` + `wizard.answer`/`wizard.choose`/`wizard.restart` + `character.create`: the multi-step draft becomes controller-held session state (recorded in `docs/decisions/wizard-session-ownership.md`), the step screen is a new semantic `wizard` `ViewState` variant (composition lifted into the controller layer, embed chrome + button welds in `wizardViewToDiscord`), and `/join` rewires to translate + paint (the frozen dispatcher's join branch keeps dead compat params + a module-level router until M9). The agent's character creation now crosses the protocol too (`seedCharacterViaProtocol` replaces the engine-direct `seedCharacter`; the seed alignment is lowercase, matching the wizard's persisted values). Contract suite extended for the five events in the same commit; bookend oracle gains transcripts 17–19 (TTL expiry, the `_userInFlight` double-click guard, the illegal-choice copy) with the existing 16 untouched.
- **Screens oracle coverage for the read-only screens (M8.0)** — a new golden-transcript suite pins 18 dispatch-level transcripts over the six screens' slash + nav arms (incl. look's null-location branch, the charless nav edges, and the character-gate reroute) so the M8.1 seam migration and the M9 rebuild diff against a byte-level baseline. Test-only; no production behaviour change.
- **The read-only screens cross the JSON seam (M8.1)** — `/look`, `/map`, `/stats`, `/backpack`, `/journal`, `/help` become the six `screen.*` events (`screen.map` carries the optional slash-arm `focus`); the compositions move into the controller layer (`src/controller/*Screen.ts`, byte-for-byte lifts incl. `renderMap` and the `resolveScene` dep, now the controller's 7th constructor arg), `GameRouter` gains no-beat branches, and the six handlers are translate + paint only. The charless nav edges unify onto the other-events "Type `/join` first." copy (the recorded M8.1 gate: exactly the five charless-nav oracle snapshots churn); `screen.help` has no no-character arm (help works charless today). Contract suite extended for all six events in the same commit (conformance, flow-order pins, negative space).
- **Smoke-run tooling (M8.5)** — the agent-player smoke runs become protocol-shaped, replayable QA instruments and the harness plays the full player lifecycle through the seam: every run records a parallel protocol log (`AGENT_PROTOCOL_OUT`) of raw `GameEvent`/`GameResponse` dispatches + nightly-tick markers (DC-S1); sessions spawn as a FRESH player through the whole `/join` wizard walk on a per-session fake id, or INHERIT an existing PC by its recorded userId (DC-S7); a typed `AgentObserver` seam bounds the harness's engine reads (DC-S4); scripted day-start greeting + stats and look-after-outcome parity beats cover the player surface (DC-S3). `npm run agent:stub` runs the scripted brain against the contract StubBackend (deterministic, token-free, CI-runnable); `npm run agent:replay` replays a recorded log through either backend asserting DC-S5 choice fidelity (every event legal for the returned view, every envelope in its allowed response set) with byte-equal envelopes — the deterministic class replays byte-for-byte. The contract suite gains the "choice fidelity (agent stream)" describe (fixture generated in-process, asserted through both backends); the smoke assertion is a permanent test; a stub transcript is committed to the M8.5 corpus (`tests/fixtures/protocol-corpus/`) for M9's replay gate.
- **Action-paths oracle coverage for the slash `/action`, `/feedback` and `/bug` dispatch paths (M9.0)** — a new golden-transcript suite pins 18 dispatch-level transcripts over the un-crossed slash `/action` arm (guards, day-job menu fallback, resume, stale, divine intervention, auto-finish + broadcast, first decision) and the `/feedback` + `/bug` slash arms, so the M9.2/M9.3 seam migrations diff against a byte-level baseline. Test-only; no production behaviour change.
- **The `character.create` contract block no longer fails at weekends** — its `created` arm returns the first-day `/hi` view, which reads `new Date()` for the weekday/weekend branch, but unlike the sibling `hi.open` block it pinned no clock, so the suite went red every Saturday and Sunday on an assertion for the weekday "Daily Work" copy. The block now pins Wednesday 2026-07-15 the same way. Test-only; the first live instance of the SF3 same-weekday-class caveat the M8.5 replay work recorded.
- **Four remaining seam gaps close ahead of the slash-path crossing (M9.1)** — the `collapse` fact now rides every outcome envelope with a post-action character, so `announceCollapse` reads through the protocol; refunded divine-intervention rolls cross as a tenth error code (`divine-intervention`) rather than being silently repainted as a normal outcome, closing a fidelity loss the agent harness has carried since M6; `/feedback` and `/bug` gain dedicated `slash-feedback`/`slash-bug` surfaces so their persisted attribution stays distinct from the outcome-button surfaces; and `getNavButtons` accepts either a `hasPendingAction` or `lastActionState` shape, clearing the way for the dispatcher's nav reads to move behind the seam. Contract suite extended per addition in the same commit; zero snapshot churn beyond the two new divine-intervention transcripts.
- **The slash `/action`, `/feedback` and `/bug` commands cross the JSON seam (M9.2)** — `commands/action.ts` collapses to translate + paint over the `menu.open`/`action.custom` events, with every combat opening frame (enemy nameplate + banded condition) now rendering correctly instead of the `Unknown foe`/`?/?` placeholder it had shown since the paths first crossed the seam (a dropped rendering input, not a rule change); `/feedback` and `/bug` ride the M9.1 `slash-feedback`/`slash-bug` surfaces. The day-job-menu composition failure now surfaces as its own byte-identical fallback screen instead of a raw internal error, and the daily-roll guard on free-text actions moves behind the seam with the flow it protects (same value, same condition), closing the slash arm — the day-job Custom… modal button doesn't wire to it yet, left for M9.3. Five declared copy/behaviour classes churn the action-oracle snapshot (interstitial verb, description clip, a stamp-order change, and two dead-edge copy unifications); the other three oracle snapshots and the corpus stay byte-identical.
- **The profanity guard moves behind the seam, closing the unfiltered slash path (M9.3)** — `checkProfanity` now runs inside the router's `action.custom` branch instead of only in the Discord modal handler, so both free-text entry points are filtered by one rule in one place. Free text typed into `/action <description>` was **unfiltered until now** and is rejected after this change; the guard's word list and its verdict are unchanged, only where it runs. The rejection paints identically on both paths (one plain ephemeral carrying the same copy), and the guard keeps its position ahead of the character check, so a player without a character who submits blocked text is still told about the language rather than the missing character.
- **The Discord dispatcher is rebuilt onto the protocol (M9.3)** — every controller call in `dispatchInteraction.ts` becomes a `router.dispatch(event, onBeat)`, so the twelve interaction leaves (day-job work, the free-text modal, action choices, the four feedback/bug modals, the nav-button `/action` menu) are translate + paint over the seam, and the file's last runtime engine and controller imports go with them: the nav-button reads move behind the seam as a `nav` fact each command hands back, and the `/sleep` idle line becomes an injected dependency. Failed feedback and bug persists still page the admin, now from a fact rather than a swallowed throw, while an action that times out after 30 minutes no longer pages anyone — the nav-button menu had begun treating that ordinary event as a fault. The screens, copy and nav bars are otherwise unchanged, including on guard rejections, on the fault screens the seam absorbs, and on `/ping`, which has no seam event and so keeps a wiring-level nav supply. **Two deliberate changes on the day-job Custom… button**, which M9.2 recorded as owed here: a player with no rolls left is now told so instead of being granted a free unpaid action, and the no-character case answers in one message instead of two. Verified against the golden-transcript oracles and the M8.5 replay corpus, which stay byte-identical across the whole crossing apart from those and the owner-signed profanity change above.
...
- **Hard things are now actually hard** *(RA-1 / v13 prompt set)* — the old ladder capped the contract at DC 18 while calling 17+ "daunting", so no observed action ever exceeded 17 and 83% of everything succeeded. The bands are re-anchored to routine 11-13, hard 16-18, daunting 20-24, and every option set must still keep one option in the routine band, so an ungeared character always has a real approach and the top of the ladder is a gamble you choose. Success rewards scale with the difficulty actually attempted, which the narrator is now told outright rather than left to infer (see the difficulty-signal entry below).
- **The narrator is now told how hard the thing you attempted was** *(P3)* - both resolve recipes were instructed to scale the reward by the difficulty attempted, and the resolve handoff carried no difficulty of any kind, so the size of a reward came down to how ambitious the chosen option's wording happened to read. A DC-checked attempt now carries the DC it was resolved against, and a fight carries the worded danger tier the combat card already shows you, so the prose and the card describe the same fight on the same scale. A fight is a contested roll with no threshold to pass, which is why it gets a tier rather than a number, and an auto-resolved action carries neither, since nothing was tested against anything (commits `4432047`, `8794796`).
