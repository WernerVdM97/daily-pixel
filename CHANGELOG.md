...
- **Edit discipline for the delegated tiers** — `orchestrated-delegation` and `changelog` now state the rules the factory's edit failures keep repeating: read before edit, one hunk per call on unique text, re-read after a PARTIAL APPLY, and anchor changelog edits on the release heading rather than a repeating kind heading.
