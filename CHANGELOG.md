...
- **Start Over on every `/join` step** — a red "Start Over" button now appears on every wizard step (1–7), not just the final review, so players can restart without abandoning the flow.
