...
- **A spec for OpenRouter key separation and cache observability (`docs/engine/openrouter-keys-and-cache-observability.md`)**: the two loose ends the route change left. It maps which actor spends on which key (interactive `pi`, the factory's 11 agents, the bot and the player-agent, two of them sharing one key today), records that per-key accounting already exists (`GET /api/v1/key` reports usage and limits) and that the factory key now carries a `$5`-a-day cap whose `limit_remaining` the launcher reads before firing, so a spent cap is a skipped tick with a line saying why rather than a 401 per call, and documents the precedence trap that makes an environment variable useless for the factory (`--api-key` beats `auth.json`, which beats the environment, verified both ways). The second half designs the five `llm_calls` columns that make the host pin provable, and notes that OpenRouter returns the serving host on the response itself, so unlike the pi-side `cachestat` pattern this needs no per-generation id lookup.
