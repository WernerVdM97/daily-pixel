...
- **The agent-player can be told to take a free action (`AGENT_FORCE_FREE_ACTIONS=1`)** — each day's first menu withholds the day-job buttons until a free-text (non-work) action completes, because the brain otherwise plays day-job work almost exclusively and `stripWorkInspiration` removes every inspiration grant on `kind: 'work'`, leaving RA-2's dial unreadable from a run. Off by default (the menu is unchanged when unset), and the run summary now prints the run's free-action count.
