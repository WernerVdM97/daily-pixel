...
/**
 * Sum item modifiers for a stat. Quantity does NOT multiply: the modifier is the item's quality,
 * quantity only tracks consumption (ammo/consumables), so ten arrows shoot no better than one.
 */
...
/**
 * Effective ability scores: base stats plus summed item modifiers. This drives rolls, so `/stats`
 * and the might leaderboard surface it — base stats alone understate a geared character.
 */
...
/**
 * Roll bonus: the character's stat plus summed item modifiers for that stat, 0 for an unknown key.
 * The stat is chosen per decision by the player's approach, so build and gear decide which approach is strongest.
 */
...
/**
 * Resolve a d20 roll against a DC: natural 1 always fails, natural 20 always succeeds, otherwise
 * `roll + bonus >= DC`.
 */
