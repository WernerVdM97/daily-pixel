...
**Item breakage / loss:**
Use `remove_item` when the player pushes gear beyond its limits. Check the `inventory:` entry in the SCALING HINT block to choose which item breaks.
...
RECENT ACTIONS (last 3, oldest first): {type + outcome, with the prior outcome narrative for continuity}
KNOWN LOCATIONS: {comma-separated list of every charted location name — prefer these exact names for set_location; see Location rules}
PLAYER INPUT: {raw_input}
SCALING HINT: {per-stat item bonuses, inventory list}
