...
- **`set_location` — travel to a place that already exists.** Use it to move to a **Charted** exit (copy its name verbatim, casing included) — or to any other already-known place the player heads back to. The engine finds the route and moves you; you name the destination and add the journey's stamina cost (`modify_stamina`, per the travel recipe above). **Never invent a name here** and never use a synonym for a place that exists ("The Temple" for "The Shrine of the First Flame").
