...
- **`set_location` — travel to a place that already exists.** Use it to move to a **Charted** exit (copy its name verbatim, casing included) — or to any other already-known place the player heads back to. The engine finds the route and charges the effort; you only name the destination. **Never invent a name here** and never use a synonym for a place that exists ("The Temple" for "The Shrine of the First Flame").
...
- **`ROLL RESULT: SUCCESS`** → the attempt worked. **You MUST include at least one positive mutation.** Choose from:
  ...
  - `set_location` — you arrive at a known place · `cross_frontier` — you break new ground down a frontier road
...
4. **Locations** — `set_location` names an existing place (a Charted exit or known location, verbatim); a NEW place is born ONLY via `cross_frontier` on a frontier `direction` from `### Exits from here`. Never invent a name for `set_location`, never `cross_frontier` a direction not listed.
...
- `### Exits from here` — the local travel menu. **Charted** exits (`direction → Name (effort N)`) are
  the places you can `set_location` to by name; **Uncharted frontier** exits (`direction — teaser (effort N)`)
