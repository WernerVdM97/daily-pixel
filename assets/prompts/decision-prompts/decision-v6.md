...
- **`NEW_ACTION`** — the first beat. Open a decision (`done: false`) or, for a trivial/auto-resolving intent (travel, rest), resolve outright (`done: true`).
...
- Lean an option's `stat` toward what the fiction implies, and let the player's sheet (shown in CHARACTER) and gear (shown in the item bonuses hint) make some approaches stronger for them than others.
...
- The bail option (`dc_modifier: null`) is always "Step back" or a narratively appropriate retreat — never skip it. The bail option needs no `stat`.
...
5. **Options** — each non-bail option has a `stat`; the mix tests at least two different stats.
...
```json
{
  ...
  "done": true | false,
  "decision": [
    { "label": "action description", "stat": "physical | wisdom | intelligence | charisma", "dc_modifier": -5 to 5, or null for bail }
  ],
...
| `decision` | when `done: false` | 2-4 options. Each has `label` (short action description), `stat` (the ability this approach tests — optional, defaults to the top-level `stat`), and `dc_modifier` (signed: negative = easier, positive = harder; null = bail/retreat — bail needs no stat). |
...
CHARACTER: {class, stats, health, stamina, alignment, day_job}  — `stats` are the ability scores added to the roll
LOCATION: {name}
NEARBY NPCS: {name + description}
NEARBY PCS: {name + class}
RECENT ACTIONS (last 2): {type + outcome summary}
PLAYER INPUT: {raw_input}
SCALING HINT: {per-stat item bonuses, inventory list, available location names}
