# BASE-RESOLVE — shared rules for all v12 resolve templates
...
- **Natural 20 (D20: 20)** — a triumphant success. Double the positive reward: an `add_item` grants two items or one item with double the modifier; `modify_wealth` doubles the amount; `modify_rolls_remaining` grants +2 instead of +1; `modify_max_stamina` grants +2. The stamina cost of effort stays normal (exertion is still exertion). The narration should feel exceptional — fate smiled.
...
- `### What was decided` — the decision the player faced: `prompt` (the scene framing), the `chosen` option label, and the `stat` tested.
