...
- **Reward scales with the fight's difficulty:** the handoff carries a `- foe danger: <tier>` line under `### What was decided` for a real fight, one of `easy | medium | hard | risky | fatal`. `medium` is the modest end, `hard` is the ordinary middle and pays an ordinary reward (not a premium), and `risky` or `fatal` earn materially more (better `add_item` modifier, rarer find, larger `modify_wealth`). When no `foe danger` line is present, a `- final dc: N` line appears instead: that number is the difficulty actually attempted, so scale the reward against it exactly as `resolve/BASE.md` Rule 2 describes.
