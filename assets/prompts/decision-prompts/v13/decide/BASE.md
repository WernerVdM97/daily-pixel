...
- **On `NEW_ACTION`**, keep the final DC honest: an option's final DC is `baseDc` + its own `dcModifier`, and it is that figure which should land ~11-13 routine, 16-18 hard, 20-24 daunting (the roll adds the character's ability score on top). **On `CONTINUE`**, the engine carries the running DC forward and `dcModifier` is the only lever, so aim for a meaningful spread (one clearly safer option, one clearly riskier) rather than re-deriving absolute bands.
...
- **`baseDc` is the anchor of the spread, not the action's difficulty.** Set it around 16-17 for an ordinary action, then choose each option's `dcModifier` (capped at ±5) so that option's own final DC (`baseDc` + `dcModifier`) lands in the band that option is meant to cover, rather than applying a flat offset from the anchor. Anchoring at the routine end leaves no headroom to reach the daunting band at all (anchor 16, options at -4/+1/+5 give final DCs 12/17/21, one in each band).
...
3. **Final DC honest** (`NEW_ACTION`): each option's `baseDc + dcModifier` lands 11-13 routine, 16-18 hard, 20-24 daunting; the anchor (`baseDc`) sits mid-spread, not at either end. On `CONTINUE`, check the spread instead: one option clearly safer, one clearly riskier. Remember the roll adds the character's stat + item bonus. Difficulty keys to each option's ambition, not one flat DC per action; every option set keeps at least one option whose final DC is in the routine band, so the daunting band is a gamble the player chooses, never a wall for the ungeared.
...
| `baseDc` | always | The anchor of the option spread, not the final difficulty. On `NEW_ACTION`, what the player rolls against is `baseDc` + the chosen option's `dcModifier`; on `CONTINUE`, the engine carries the running DC forward instead and the newly authored `baseDc` is not used. Legal range 10-24, anchored around 16-17 so a routine option can sit at 11-13 and an ambitious one at 20-24 within the ±5 cap. Remember the roll adds the character's stat + item bonus. |
